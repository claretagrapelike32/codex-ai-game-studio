...
| Engine-specific patterns or APIs | Primary engine specialist (from `.ai-game-studio/project.json`) |
...
actual subagent spawns. A simulated review is not a specialist review.**

Issue all subagent spawns simultaneously. Do NOT spawn one at a time.
...
Use `the available user-input mechanism` for ALL closing interactions. Never plain text.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab `the available user-input mechanism` before making any edits — do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker → fix applied) and use `the available user-input mechanism` for a **post-revision closing widget**:
...
- Options:
  - `[A] Re-review in a new session — run $ai-game-studio:design-review [doc-path] after /clear`
  - `[B] Accept revisions and mark Approved — update systems index, skip re-review`
  - `[C] Move to next system — $ai-game-studio:design-system [next-system] (#N in design order)`
  - `[D] Stop here`
...
When the verdict is APPROVED, use a single `the available user-input mechanism` with `multiSelect: true` to batch the two tracking updates:
...
Use a second `the available user-input mechanism`:
...
Use a third `the available user-input mechanism`:
...
Once the systems-index and review-log widgets are answered, check project state and show one final `the available user-input mechanism`:
...
- Count `.md` files in `design/gdd/` (excluding game-concept.md, systems-index.md) to determine if `$ai-game-studio:review-all-gdds` is worth offering (≥2 GDDs)
...
- `[_] Run $ai-game-studio:design-review [other-gdd-path] — [system name] is still [In Review / NEEDS REVISION]` (include if another GDD needs review)
- `[_] Run $ai-game-studio:consistency-check — verify this GDD's values don't conflict with existing GDDs` (always include if ≥1 other GDD exists)
- `[_] Run $ai-game-studio:review-all-gdds — holistic design-theory review across all designed systems` (include if ≥2 GDDs exist)
- `[_] Run $ai-game-studio:design-system [next-system] — next in design order` (always include, name the actual system)
- `[_] Stop here`
