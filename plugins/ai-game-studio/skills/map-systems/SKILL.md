...
- **No argument**: `$ai-game-studio:map-systems` — Run the full decomposition workflow (Phases 1-5)
  to create or update the systems index.
- **`next`**: `$ai-game-studio:map-systems next` — Pick the highest-priority undesigned system
  from the index and hand off to `$ai-game-studio:design-system` (Phase 6).
...
- Read `design/gdd/game-concept.md` — **fail with a clear message if missing**:
  > "No game concept found at `design/gdd/game-concept.md`. Run `$ai-game-studio:brainstorm` first
...
- file discovery `design/gdd/*.md` — check which system GDDs already exist

**If the systems index already exists:**
- Read it and present current status to the user
- Use `the available user-input mechanism` to ask:
...
Then use `the available user-input mechanism` to capture feedback:
...
Use `the available user-input mechanism` to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` with a Codex subagent using gate TD-SYSTEM-BOUNDARY (the installed Codex role-gate protocol) before proceeding to priority assignment.**
...
Use `the available user-input mechanism` to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` with a Codex subagent using gate PR-SCOPE (the installed Codex role-gate protocol) before writing the index.**
...
Using the template at `.ai-game-studio/templates/systems-index.md`, populate the
...
**After the systems index is written, spawn `creative-director` with a Codex subagent using gate CD-SYSTEMS (the installed Codex role-gate protocol).**
...
## Phase 6: Design Individual Systems (Handoff to $ai-game-studio:design-system)

This phase is entered when:
- The user says "yes" to designing systems after creating the index
- The user invokes `$ai-game-studio:map-systems [system-name]`
- The user invokes `$ai-game-studio:map-systems next`
...
Use `the available user-input mechanism` for: "Start designing [system-name] now, pick a different
system, or stop here?"

### Step 6b: Hand Off to $ai-game-studio:design-system

Once a system is selected, invoke the `$ai-game-studio:design-system [system-name]` skill.

The `$ai-game-studio:design-system` skill handles the full GDD authoring process:
...
- Runs `$ai-game-studio:design-review` when complete
- Updates the systems index

**Do not duplicate the $ai-game-studio:design-system workflow here.** This skill owns the systems
*index*; `$ai-game-studio:design-system` owns individual system *GDDs*.

### Step 6c: Loop or Stop

After `$ai-game-studio:design-system` completes, use `the available user-input mechanism`:
...
After the systems index is created (or after designing some systems), present next actions using `the available user-input mechanism`:

- "Systems index is written. What would you like to do next?"
  - [A] Start designing GDDs — run `$ai-game-studio:design-system [first-system-in-order]`
  - [B] Run `$ai-game-studio:gate-check systems-design` — triggers the CD-SYSTEMS and TD-SYSTEM-BOUNDARY gates automatically for a formal director sign-off on the system set
  - [C] Stop here for this session

**The gate-check option ([B]) is worth highlighting**: running `$ai-game-studio:gate-check systems-design` triggers both the CD-SYSTEMS and TD-SYSTEM-BOUNDARY gates, catching scope issues, missing systems, and boundary problems before they're locked in across many documents. It is optional but recommended for new projects.

After any individual GDD is completed:
- "Run `$ai-game-studio:design-review design/gdd/[system].md` in a fresh session to validate quality"
- "Run `$ai-game-studio:gate-check systems-design` when all MVP GDDs are complete"
...
2. **the available user-input mechanism** at every decision point (Explain -> Capture pattern):
   ...
   - Phase 6: "Start designing, pick different, or stop?" then hand off to `$ai-game-studio:design-system`
...
5. **Handoff**: Individual GDD authoring is owned by `$ai-game-studio:design-system`, which handles
...
> designing individual GDDs — run `$ai-game-studio:map-systems next` to pick up where you left off.

---

## Recommended Next Steps

- Run `$ai-game-studio:design-system [first-system-in-order]` to author the first GDD (use design order from the index)
- Run `$ai-game-studio:map-systems next` to always pick the highest-priority undesigned system automatically
- Run `$ai-game-studio:design-review design/gdd/[system].md` in a fresh session after each GDD is authored
