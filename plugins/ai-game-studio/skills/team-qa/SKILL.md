...
**Decision Points:** At each phase transition, use `the available user-input mechanism` to present
...
1. Detect the current sprint or feature scope from the argument:
   - If argument is a sprint identifier (e.g., `sprint-03`): file discovery `production/sprints/` for files matching `*[sprint-identifier]*.md`. Read the matched file. If multiple match, use the most recently modified.
...
Spawn `qa-lead` with a Codex subagent to review all in-scope stories and produce a QA strategy.
...
- **Before assessing smoke status, check for an existing smoke check report**: file discovery `production/qa/smoke-*.md` and read the most recently modified file (if found). If a report exists, use its verdict and findings directly — do not re-interview the user. If no report exists, note: "No prior smoke check report found — run `$ai-game-studio:smoke-check sprint` before proceeding." and set smoke check status to UNKNOWN (treat as PASS WITH WARNINGS for the purpose of continuing). Produce a smoke check verdict: **PASS** / **PASS WITH WARNINGS [list]** / **FAIL [list of failures]** / **UNKNOWN (no report found)**
...
Present the qa-lead's full strategy to the user, then use `the available user-input mechanism`:
...
options:
  ...
  - "Smoke check failed — fix issues and re-run $ai-game-studio:team-qa"
  - "Cancel — resolve blockers first"
```

If smoke check **FAIL**: do not proceed to Phase 3. Surface the failures from the smoke check report and stop. The user must fix them, re-run `$ai-game-studio:smoke-check sprint`, and then re-run `$ai-game-studio:team-qa`.
If smoke check **UNKNOWN**: surface a warning — "No smoke check report found. Recommend running `$ai-game-studio:smoke-check sprint` before QA. Proceeding with caution."
...
Spawn `qa-tester` with a Codex subagent for each story (run in parallel where possible), providing:
- The story file path
...
Use `the available user-input mechanism` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `the available user-input mechanism` for each:
...
After each FAIL result: use `the available user-input mechanism` to collect the failure description, then spawn `qa-tester` with a Codex subagent to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` with a Codex subagent to produce the sign-off report using all results from Phases 4–6.
...
- APPROVED: "Build is ready for the next phase. Run `$ai-game-studio:gate-check` to validate advancement."
- APPROVED WITH CONDITIONS: "Resolve conditions before advancing. S3/S4 bugs may be deferred to polish."
- NOT APPROVED: "Resolve S1/S2 bugs and re-run `$ai-game-studio:team-qa` or targeted manual QA before advancing."
