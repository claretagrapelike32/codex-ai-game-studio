---
name: team-ui
description: "Orchestrate the UI team through the full UX pipeline: from UX spec authoring through visual design, implementation, review, and polish. Integrates with $ai-game-studio:ux-design, $ai-game-studio:ux-review, and studio UX templates."
---
...
- **engine UI specialist** — Validates UI implementation patterns against engine-specific best practices (read from `.ai-game-studio/project.json` Engine Specialists → UI Specialist)
...
Then use `the available user-input mechanism` with options:
- (a) Run `$ai-game-studio:ux-design patterns` first to establish the pattern library, then continue
...
Invoke `$ai-game-studio:ux-design [feature name]` skill OR delegate directly to ux-designer to produce `design/ux/[feature-name].md` following the `ux-spec.md` template.

If designing the HUD, use the `hud-design.md` template instead of `ux-spec.md`.

> **Notes on special cases:**
> - For HUD design specifically, invoke `$ai-game-studio:ux-design` with `argument: hud` (e.g., `$ai-game-studio:ux-design hud`).
> - For the interaction pattern library, run `$ai-game-studio:ux-design patterns` once at project start and update it whenever new patterns are introduced during later phases.

Output: `design/ux/[feature-name].md` with all required spec sections filled.

### Phase 1c: UX Review

After the spec is complete, invoke `$ai-game-studio:ux-review design/ux/[feature-name].md`.

**Gate**: Do not proceed to Phase 2 until the verdict is APPROVED. If the verdict is NEEDS REVISION, the ux-designer must address the flagged issues and re-run the review. The user may explicitly accept a NEEDS REVISION risk and proceed, but this must be a conscious decision — present the specific concerns via `the available user-input mechanism` before asking whether to proceed.
...
Before implementation begins, spawn the **engine UI specialist** (from `.ai-game-studio/project.json` Engine Specialists → UI Specialist) to review the UX spec and visual design spec for engine-specific implementation guidance:
...
- `$ai-game-studio:ux-design` — Author a new UX spec for a screen, flow, or HUD from scratch
- `$ai-game-studio:ux-review` — Validate a completed UX spec before implementation
- `$ai-game-studio:team-ui [feature]` — Full pipeline from concept through polish (calls `$ai-game-studio:ux-design` and `$ai-game-studio:ux-review` internally)
- `$ai-game-studio:quick-design` — Small UI changes that don't need a full new UX spec
...
delegated to sub-agents and sub-skills (`$ai-game-studio:ux-design`, `ui-programmer`). Each enforces the
...
- Run `$ai-game-studio:ux-review` on the final spec if not yet approved.
- Run `$ai-game-studio:code-review` on the UI implementation before closing stories.
- Run `$ai-game-studio:team-polish` if visual or audio polish pass is needed.
