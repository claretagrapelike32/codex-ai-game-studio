...
**This skill is the write-time safety net.** It catches what `$ai-game-studio:design-system`'s
per-section checks may have missed and what `$ai-game-studio:review-all-gdds`'s holistic review
catches too late.

**When to run:**
- After writing each new GDD (before moving to the next system)
- Before `$ai-game-studio:review-all-gdds` (so that skill starts with a clean baseline)
- Before `$ai-game-studio:create-architecture` (inconsistencies poison downstream ADRs)
- On demand: `$ai-game-studio:consistency-check entity:[name]` to check one entity specifically
...
> "Entity registry is empty. Run `$ai-game-studio:design-system` to write GDDs — the registry
...
```
file discovery pattern="design/gdd/*.md"
```
...
## Phase 3: text search-First Conflict Scan
...
```
text search pattern="[entity_name]" glob="design/gdd/*.md" output_mode="content" -C 3
```
...
```
(Or use text search with wider context if the file is large)
...
```markdown
### [YYYY-MM-DD] — $ai-game-studio:consistency-check — 🔴 CONFLICT
...
<!-- Auto-maintained by $ai-game-studio:consistency-check. Do not edit manually. -->
...
Then close with an `the available user-input mechanism` widget:
...
- **Options**:
  ...
  - `[C] Run $ai-game-studio:design-review on the most conflicted GDD`
  - `[D] Stop here`
...
- **If PASS**: Run `$ai-game-studio:review-all-gdds` for holistic design-theory review, or
  `$ai-game-studio:create-architecture` if all MVP GDDs are complete.
- **If CONFLICTS FOUND**: Fix the flagged GDDs, then re-run
  `$ai-game-studio:consistency-check` to confirm resolution.
- **If STALE REGISTRY**: Update the registry (Phase 6), then re-run to verify.
- Run `$ai-game-studio:consistency-check` after writing each new GDD to catch issues early,
