...
See the installed Codex role-gate protocol for the full check pattern and mode definitions.

---

## 1. Parse Arguments

**Scope:** `the first invocation argument` (blank = ask user via the available user-input mechanism)

- **Specific path** (e.g., `$ai-game-studio:story-readiness production/epics/combat/story-001-basic-attack.md`):
...
If no argument is given, use `the available user-input mechanism`:
...
- [ ] **TR-ID is valid and active**: If the story contains a `TR-[system]-NNN`
  ...
  - If the ID does not exist in the registry → NEEDS WORK: ID was not registered
    (story may predate registry, or registry needs an `$ai-game-studio:architecture-review` run).
...
- [ ] **Referenced assets exist**: Scan the story text for asset path patterns
  ...
  - For each asset path found: use file discovery to check whether the file exists.
...
Resolve these before the sprint begins or replan with `$ai-game-studio:sprint-plan update`.
```
...
in conversation. Do not use the available file-editing mechanism — the user (or
`$ai-game-studio:create-stories`) handles writing.

**Redirect rules:**
- If a story file does not exist at all: "This story file is missing entirely.
  Run `$ai-game-studio:create-epics [layer]` then `$ai-game-studio:create-stories [epic-slug]` to generate stories from the GDD and ADR."
- If a story has no GDD reference and the work appears small: "This story has
  ...
  `$ai-game-studio:quick-design [description]` to create a Quick Design Spec, then reference
...
Run `$ai-game-studio:story-readiness [path]` to validate before starting.
```
...
Spawn `qa-lead` with a Codex subagent using gate **QL-STORY-READY** (the installed Codex role-gate protocol).
...
- **GAPS [list]** → surface the specific gaps to the user via `the available user-input mechanism`:
...
- Run `$ai-game-studio:dev-story [story-path]` to begin implementation once the story is READY
- Run `$ai-game-studio:story-readiness sprint` to check all stories in the current sprint at once
- Run `$ai-game-studio:create-stories [epic-slug]` if a story file is missing entirely
