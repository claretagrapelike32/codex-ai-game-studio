...
- **Design changes:** "Run `$ai-game-studio:propagate-design-change [path]` on the affected design document to find downstream impacts before making changes."
- **Balance adjustments:** "Run `$ai-game-studio:balance-check [system]` to verify the full balance picture before tuning values."
- **Bugs:** "Use `$ai-game-studio:bug-report` to formally track these."
...
After categorising findings, spawn `creative-director` with a Codex subagent using gate **CD-PLAYTEST** (the installed Codex role-gate protocol).
...
- After addressing design changes: re-run `$ai-game-studio:design-review` on the updated GDD.
- After fixing bugs: re-run `$ai-game-studio:bug-triage` to update priorities.
