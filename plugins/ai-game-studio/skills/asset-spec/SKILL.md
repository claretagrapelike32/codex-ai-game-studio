...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use `the available user-input mechanism`:
...
Present the full proposed inventory to the user in conversation. Then use `the available user-input mechanism`:
...
> - Run `$ai-game-studio:ux-design [screen name]` for each UI screen in the inventory
> - Run `$ai-game-studio:asset-spec entity:[name]` to spec each visual entity
> - Or run `$ai-game-studio:asset-spec` again to work through the inventory one item at a time"
...
- **Art bible**: Read `design/art/art-bible.md` — fail if missing:
  > "No art bible found. Run `$ai-game-studio:art-bible` first — asset specs are anchored to the art bible's visual rules and asset standards."
...
- **Technical preferences**: Read `.ai-game-studio/project.json` — extract performance budgets and naming conventions.
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  > "The Visual/Audio section of `design/gdd/[target-name].md` is empty. Either run `$ai-game-studio:design-system [target-name]` to complete the GDD, or describe the visual needs manually."
  Use `the available user-input mechanism`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `the available user-input mechanism`:
...
- **Related specs**: file discovery `design/assets/specs/*.md` — scan for assets that could be shared (e.g., a common UI element specced for one system might apply here too).
...
Present the full identified list to the user. Use `the available user-input mechanism`:
...
Spawn specialist agents based on review mode. **Issue all subagent spawns simultaneously — do not wait for one before starting the next.**

### Full mode — spawn in parallel:

**`art-director`** with a Codex subagent:
...
**`technical-artist`** with a Codex subagent:
...
After presenting all specs, use `the available user-input mechanism`:
...
- Options:
  - `[A] Spec another system — $ai-game-studio:asset-spec system:[next-system]`
  - `[B] Spec a level — $ai-game-studio:asset-spec level:[level-name]`
  - `[C] Spec a character — $ai-game-studio:asset-spec character:[character-name]`
  - `[D] Run $ai-game-studio:asset-audit — validate delivered assets against specs`
  - `[E] Stop here`
...
```
text search pattern="ASSET-" path="design/assets/asset-manifest.md"
```
...
- Run `$ai-game-studio:asset-spec [next-context]` to continue speccing remaining systems, levels, or characters
- Run `$ai-game-studio:asset-audit` to validate delivered assets against the written specs and identify gaps or mismatches
