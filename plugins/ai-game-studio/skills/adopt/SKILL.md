...
**This is not `$ai-game-studio:project-stage-detect`.**
`$ai-game-studio:project-stage-detect` answers: *what exists?*
`$ai-game-studio:adopt` answers: *will what exists actually work with the template's skills?*
...
- `.ai-game-studio/project.json` — engine configured?
- `docs/engine-reference/` — engine reference docs present?
- file discovery `docs/adoption-plan-*.md` — note the filename of the most recent prior plan if any exist

### Infer phase (if no stage.txt)
Use the same heuristic as `$ai-game-studio:project-stage-detect`:
...
- Nothing → Fresh (not a brownfield project — suggest `$ai-game-studio:start`)

If the project appears fresh (no artifacts at all), use `the available user-input mechanism`:
- "This looks like a fresh project — no existing artifacts found. `$ai-game-studio:adopt` is for
  projects with work to migrate. What would you like to do?"
  - "Run `$ai-game-studio:start` — begin guided first-time onboarding"
...
|---|---|
| `## Status` | **BLOCKING** — `$ai-game-studio:story-readiness` ADR status check silently passes everything |
| `## ADR Dependencies` | HIGH — dependency ordering in `$ai-game-studio:architecture-review` breaks |
...
1. **Parenthetical status values** — text search for any Status cell containing
   parentheses: `"Needs Revision ("`, `"In Progress ("`, etc.
   These break exact-string matching in `$ai-game-studio:gate-check`, `$ai-game-studio:create-stories`,
   and `$ai-game-studio:architecture-review`. **BLOCKING.**
...
| Sprint status | `production/sprint-status.yaml` | MEDIUM — `$ai-game-studio:sprint-status` falls back to markdown |
...
Read `.ai-game-studio/project.json`. Check each field for `[TO BE CONFIGURED]`:
...
`$ai-game-studio:architecture-decision retrofit docs/architecture/adr-[NNNN]-[slug].md`
...
`$ai-game-studio:design-system retrofit design/gdd/[filename].md`
...
2. Run `$ai-game-studio:architecture-review` → bootstraps `tr-registry.yaml`
3. Run `$ai-game-studio:create-control-manifest` → creates manifest with version stamp
4. Run `$ai-game-studio:sprint-plan update` → creates `sprint-status.yaml`
5. Run `$ai-game-studio:gate-check [phase]` → writes `stage.txt` authoritatively
...
Gap counts:
  ...
  HIGH:     [N] — unsafe to run $ai-game-studio:create-stories or $ai-game-studio:story-readiness
...
Re-run `$ai-game-studio:adopt` anytime to check remaining gaps.
...
Run `$ai-game-studio:architecture-review` — even if ADRs already exist, this run bootstraps
...
Run `$ai-game-studio:create-control-manifest`
**Time**: 30 min
- [ ] docs/architecture/control-manifest.md created

### 3c. Create sprint tracking file
Run `$ai-game-studio:sprint-plan update`
...
Run `$ai-game-studio:gate-check [current-phase]`
**Time**: 5 min
...
Run `$ai-game-studio:adopt` again after completing Step 3 to verify all blocking and high gaps
...
**If it does not exist**: Use `the available user-input mechanism`:
...
- **Options**:
  ...
  - `Lean (recommended)` — Directors only at phase gate transitions ($ai-game-studio:gate-check). Skips per-skill reviews. Balanced for solo devs and small teams.
...
and offer to handle it immediately using `the available user-input mechanism`. Choose the first
branch that applies:
...
- "The most urgent fix is `systems-index.md` — [N] rows have parenthetical status
  values (e.g. `Needs Revision (see notes)`) that break $ai-game-studio:gate-check,
  $ai-game-studio:create-stories, and $ai-game-studio:architecture-review right now. I can fix these in-place."
...
- "The most urgent fix is adding `## Status` to [N] ADR(s): [list filenames].
  Without it, $ai-game-studio:story-readiness silently passes all ADR checks. Start with
...
- "The most urgent gap is missing Acceptance Criteria in [N] GDD(s):
  [list filenames]. Without them, $ai-game-studio:create-stories can't generate stories.
...
- "No blocking gaps — this project is template-compatible. What next?"
  ...
  - "Run $ai-game-studio:project-stage-detect for a broader health check"
  - "Done — I'll work through the plan at my own pace"

> **Adoption plan saved to `docs/adoption-plan-[date].md`.** Re-run `$ai-game-studio:adopt` at any time to re-check remaining gaps as you complete them.
