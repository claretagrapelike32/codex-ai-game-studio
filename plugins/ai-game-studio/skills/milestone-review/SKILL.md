...
Before generating the Go/No-Go recommendation, spawn `producer` with a Codex subagent using gate **PR-MILESTONE** (the installed Codex role-gate protocol).
...
If OFF TRACK, use `the available user-input mechanism` before generating the recommendation:
...
If AT RISK, use `the available user-input mechanism`:
...
- Run `$ai-game-studio:gate-check` for a formal phase gate verdict if this milestone marks a development phase boundary.
- Run `$ai-game-studio:sprint-plan` to adjust the next sprint based on the scope recommendations above.
