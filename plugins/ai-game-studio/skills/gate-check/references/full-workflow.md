...
**Distinct from `$ai-game-studio:project-stage-detect`**: That skill is diagnostic ("where are we?").
...
- **With argument**: `$ai-game-studio:gate-check production` — validate readiness for that specific phase
- **No argument**: Auto-detect current stage using the same heuristics as
  `$ai-game-studio:project-stage-detect`, then **confirm with the user before running**:
...
- [ ] Concept prototype exists in `prototypes/` with a REPORT.md showing PROCEED verdict
      (`$ai-game-studio:prototype [core-mechanic]`) — skipping this means GDDs may be written for an
      idea that hasn't been played. Acceptable if the concept is proven by other means.

**Quality Checks:**
- [ ] Game concept has been reviewed (`$ai-game-studio:design-review` verdict not MAJOR REVISION NEEDED)
...
- [ ] All MVP-tier GDDs exist in `design/gdd/` and individually pass `$ai-game-studio:design-review`
- [ ] A cross-GDD review report exists in `design/gdd/` (from `$ai-game-studio:review-all-gdds`)

**Quality Checks:**
- [ ] All MVP GDDs pass individual design review (8 required sections, no MAJOR REVISION NEEDED verdict)
- [ ] `$ai-game-studio:review-all-gdds` verdict is not FAIL (cross-GDD consistency and design theory checks pass)
- [ ] All cross-GDD consistency issues flagged by `$ai-game-studio:review-all-gdds` are resolved or explicitly accepted
...
- [ ] Technical preferences configured (`.ai-game-studio/project.json` populated)
...
- [ ] `$ai-game-studio:architecture-review` has been run (a review report file exists in `docs/architecture/`)
...
- [ ] `$ai-game-studio:architecture-review` engine audit shows no deprecated API usage
...
**Required Artifacts:**
- [ ] Vertical slice exists in `prototypes/` with a REPORT.md (run `$ai-game-studio:vertical-slice`) — **recommended, not blocking**; if absent, surface as CONCERNS
...
- [ ] Entity inventory exists at `design/assets/entity-inventory.md` (recommended — run `$ai-game-studio:asset-spec` with no arguments to generate collaboratively from GDDs + art bible)
...
- [ ] Control manifest exists at `docs/architecture/control-manifest.md`
      (generated by `$ai-game-studio:create-control-manifest` from Accepted ADRs)
- [ ] Epics defined in `production/epics/` with at least Foundation and Core
      layer epics present (use `$ai-game-studio:create-epics layer: foundation` and
      `$ai-game-studio:create-epics layer: core` to create them, then `$ai-game-studio:create-stories [epic-slug]`
      for each epic)
...
- [ ] All key screen UX specs have passed `$ai-game-studio:ux-review` (verdict APPROVED or NEEDS REVISION accepted)
...
- [ ] Manual validation confirms GDDs + architecture + epics are coherent
      (run `$ai-game-studio:review-all-gdds` and `$ai-game-studio:architecture-review` if not done recently)
...
- [ ] QA plan exists in `production/qa/` (generated by `$ai-game-studio:qa-plan`) covering this sprint or final production sprint
- [ ] At least one QA plan exists in `production/qa/` covering this production phase — run `$ai-game-studio:qa-plan` if missing (CONCERNS — advisory, not blocking)
- [ ] QA sign-off report exists in `production/qa/` (generated by `$ai-game-studio:team-qa`) with verdict APPROVED or APPROVED WITH CONDITIONS
...
**Quality Checks:**
- [ ] Tests are passing (run test suite with the available shell)
...
- [ ] QA test plan exists (`$ai-game-studio:qa-plan` output in `production/qa/`)
- [ ] QA sign-off report exists (`$ai-game-studio:team-qa` output — APPROVED or APPROVED WITH CONDITIONS)
...
- [ ] Balance data has been reviewed (`$ai-game-studio:balance-check` run)
- [ ] Release checklist completed (`$ai-game-studio:release-checklist` or `$ai-game-studio:launch-checklist` run)
...
### Artifact Checks
- Use `file discovery` and `Read` to verify files exist and have meaningful content
...
Use `file discovery('design/gdd/gdd-cross-review-*.md')` to find the `$ai-game-studio:review-all-gdds` report.
If no file matches, mark the "cross-GDD review report exists" artifact as **FAIL** and
surface it prominently: "No `$ai-game-studio:review-all-gdds` report found in `design/gdd/`. Run
`$ai-game-studio:review-all-gdds` before advancing to Technical Setup."
...
### Quality Checks
- For test checks: Run the test suite with the available shell if a test runner is configured
...
- For performance checks: `Read` technical-preferences.md and compare against any
  profiling data in `tests/performance/` or recent `$ai-game-studio:perf-profile` output
- For localization checks: `text search` for hardcoded strings in `src/`
...
- "Performance profiling data isn't available. Would you like to run `$ai-game-studio:perf-profile`?"
...
Before generating the final verdict, spawn all four directors as **parallel subagents** with a Codex subagent using the parallel gate protocol from the installed Codex role-gate protocol. Issue all four subagent spawns simultaneously — do not wait for one before starting the next.

**Spawn in parallel:**

1. **`creative-director`** — gate **CD-PHASE-GATE** (the installed Codex role-gate protocol)
2. **`technical-director`** — gate **TD-PHASE-GATE** (the installed Codex role-gate protocol)
3. **`producer`** — gate **PR-PHASE-GATE** (the installed Codex role-gate protocol)
4. **`art-director`** — gate **AD-PHASE-GATE** (the installed Codex role-gate protocol)
...
### Blockers
1. **No Architecture Decision Records** — Run `$ai-game-studio:architecture-decision` to create one
...
> **Tool-action requirement**: At least 2 of the 5 challenge questions below must be answered by re-reading a specific file (Read tool) or re-running a specific check (text search tool) — not by reflection alone. Mark these with [TOOL ACTION] to indicate a tool was used.
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `the available user-input mechanism`.
...
[A] Run $ai-game-studio:create-architecture — produce your master architecture blueprint and ADR work plan (recommended next step)
[B] Design more GDDs first — return here when all MVP systems are complete
[C] Stop here for this session
```

> **Note for systems-design PASS**: `$ai-game-studio:create-architecture` is the required next step before writing any ADRs. It produces the master architecture document and a prioritized list of ADRs to write. Running `$ai-game-studio:architecture-decision` without this step means writing ADRs without a blueprint — skip it at your own risk.

For **technical-setup PASS**:
```
Gate passed. What would you like to do next?
[A] Run $ai-game-studio:create-control-manifest — generate the layer rules manifest from your Accepted ADRs (do this first)
[B] Run $ai-game-studio:vertical-slice — build the Vertical Slice (do this before writing epics — validate fun first)
[C] Write more ADRs first — run $ai-game-studio:architecture-decision [next-system]
...
>
> 1. `$ai-game-studio:create-control-manifest` — extract technical rules from Accepted ADRs (required before epics)
> 2. `$ai-game-studio:vertical-slice` — build the Vertical Slice **FIRST**, before writing epics or stories
> 3. Playtest → `$ai-game-studio:playtest-report` — at least 1 session required to pass the Pre-Production gate; 3+ recommended before committing the full team
> 4. `$ai-game-studio:ux-design [screen]` — UX specs for main menu, core HUD, pause menu (if not done)
> 5. `$ai-game-studio:create-epics layer:foundation` then `$ai-game-studio:create-epics layer:core` — plan after fun is validated
> 6. `$ai-game-studio:create-stories [epic-slug]` for each epic
> 7. `$ai-game-studio:sprint-plan new`
>
...
- **No art bible?** → `$ai-game-studio:art-bible` to create the visual identity specification
- **Art bible exists but no asset specs?** → `$ai-game-studio:asset-spec system:[name]` to generate per-asset visual specs and generation prompts from approved GDDs
- **No game concept?** → `$ai-game-studio:brainstorm` to create one
- **No systems index?** → `$ai-game-studio:map-systems` to decompose the concept into systems
- **Missing design docs?** → `$ai-game-studio:reverse-document` or delegate to `game-designer`
- **Small design change needed?** → `$ai-game-studio:quick-design` for changes under ~4 hours (bypasses full GDD pipeline)
- **No UX specs?** → `$ai-game-studio:ux-design [screen name]` to author specs, or `$ai-game-studio:team-ui [feature]` for full pipeline
- **UX specs not reviewed?** → `$ai-game-studio:ux-review [file]` or `$ai-game-studio:ux-review all` to validate
- **No accessibility requirements doc?** → run `$ai-game-studio:ux-design` which creates both `design/accessibility-requirements.md` and `design/ux/interaction-patterns.md` in one step
- **No interaction pattern library?** → `$ai-game-studio:ux-design patterns` to initialize it
- **GDDs not cross-reviewed?** → `$ai-game-studio:review-all-gdds` (run after all MVP GDDs are individually approved)
- **Cross-GDD consistency issues?** → fix flagged GDDs, then re-run `$ai-game-studio:review-all-gdds`
- **No test framework?** → `$ai-game-studio:test-setup` to scaffold the framework for your engine
- **No QA plan for current sprint?** → `$ai-game-studio:qa-plan sprint` to generate one before implementation begins
- **Missing ADRs?** → `$ai-game-studio:architecture-decision` for individual decisions
- **No master architecture doc?** → `$ai-game-studio:create-architecture` for the full blueprint
- **ADRs missing engine compatibility sections?** → Re-run `$ai-game-studio:architecture-decision`
  or manually add Engine Compatibility sections to existing ADRs
- **Missing control manifest?** → `$ai-game-studio:create-control-manifest` (requires Accepted ADRs)
- **Missing epics?** → `$ai-game-studio:create-epics layer: foundation` then `$ai-game-studio:create-epics layer: core` (requires control manifest)
- **Missing stories for an epic?** → `$ai-game-studio:create-stories [epic-slug]` (run after each epic is created)
- **Stories not implementation-ready?** → `$ai-game-studio:story-readiness` to validate stories before developers pick them up
- **Tests failing?** → delegate to `lead-programmer` or `qa-tester`
- **No playtest data?** → `$ai-game-studio:playtest-report`
- **No playtest sessions beyond the minimum?** → Additional sessions give more reliable signal. 3+ total is recommended before committing the full team. Use `$ai-game-studio:playtest-report` to structure findings.
- **No Difficulty Curve doc?** → Create `design/difficulty-curve.md` from the template at `.ai-game-studio/templates/difficulty-curve.md` — or use `$ai-game-studio:quick-design "difficulty curve"` for a guided session.
- **No player journey map?** → Create `design/player-journey.md` from the template at `.ai-game-studio/templates/player-journey.md` — or author it collaboratively using `$ai-game-studio:ux-design` Phase 2b.
- **Need a quick sprint check?** → `$ai-game-studio:sprint-status` for current sprint progress snapshot
- **Performance unknown?** → `$ai-game-studio:perf-profile`
- **Not localized?** → `$ai-game-studio:localize`
- **Ready for release?** → `$ai-game-studio:launch-checklist`
...
6. **Never auto-fix**: If required artifacts are missing, report the FAIL verdict and
   name the skill to run (e.g. "run `$ai-game-studio:test-setup`"). Do NOT create missing files or
