...
- After `$ai-game-studio:team-qa` completes and new bugs have been filed
...
**Modes:**
- `$ai-game-studio:bug-triage sprint` — triage against the current sprint; assign fixable bugs
  to the sprint backlog; defer the rest
- `$ai-game-studio:bug-triage full` — full triage of all bugs regardless of sprint scope
- `$ai-game-studio:bug-triage trend` — trend analysis only (no assignment); read-only report
...
file discovery for bug reports in priority order:
...
Read the nearest applicable `AGENTS.md` coding guidance for severity/priority definitions if they
...
> **Generated by**: $ai-game-studio:bug-triage
...
- If any S1 bugs are unassigned: "S1 bugs must be assigned before the sprint
  can be considered healthy. Run `$ai-game-studio:sprint-status` to see current capacity."
- If regression bugs exist: "Regressions found — consider re-opening the
  affected stories in sprint tracking and running `$ai-game-studio:smoke-check` to re-gate."
