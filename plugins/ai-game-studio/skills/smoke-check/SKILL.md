...
1. **Test framework check**: verify `tests/` directory exists.
   If it does not: "No test directory found at `tests/`. Run `$ai-game-studio:test-setup`
...
3. **Engine detection**: read `.ai-game-studio/project.json` and
...
5. **QA plan check**: glob `production/qa/qa-plan-*.md` and take the most
   ...
   `$ai-game-studio:qa-plan sprint` before smoke-checking for best results."
...
Attempt to run the test suite with the available shell. Select the command based on the engine
detected in Phase 1:
...
"Engine not configured in `.ai-game-studio/project.json`. Run
`$ai-game-studio:setup-engine` to specify the engine, then re-run `$ai-game-studio:smoke-check`."
...
3. If the `quick` argument was passed, skip this phase entirely and note:
   "Coverage scan skipped — run `$ai-game-studio:smoke-check sprint` for full coverage
   analysis."
...
2. file discovery `tests/unit/[system]/` and `tests/integration/[system]/` for files
...
appear prominently in the report and must be resolved before `$ai-game-studio:story-done` can
...
Use `the available user-input mechanism` to batch-verify. Keep to at most 3 calls.
...
**Engine**: [engine]
**QA Plan**: [path, or "Not found — run $ai-game-studio:qa-plan first"]
...
Fix the failures and run `$ai-game-studio:smoke-check` again to re-gate before QA hand-off."

**If verdict is PASS WITH WARNINGS:**

"Smoke check passed with warnings. The build is ready for manual QA.

Advisory items to resolve before running `$ai-game-studio:story-done` on affected stories:
...
- **PASS WITH WARNINGS does not block QA hand-off** — it records advisory
  gaps for `$ai-game-studio:story-done` to follow up on.
- **`quick` argument** skips Phase 3 (coverage scan) and Phase 4 Batch 3.
  Use it for rapid re-checks after fixing a specific failure.
- Use `the available user-input mechanism` for all manual smoke check verification.
