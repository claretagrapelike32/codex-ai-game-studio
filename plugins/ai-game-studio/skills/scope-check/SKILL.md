...
- **CONCERNS** → offer to identify the 2–3 additions with best cut ratio. Reference `$ai-game-studio:sprint-plan update` to formally re-scope.
- **FAIL** → recommend escalating to producer. Reference `$ai-game-studio:sprint-plan update` for re-planning or `$ai-game-studio:estimate` to re-baseline timeline.

Always end with:
> "Run `$ai-game-studio:scope-check [name]` again after cuts are made to verify the verdict improves."
