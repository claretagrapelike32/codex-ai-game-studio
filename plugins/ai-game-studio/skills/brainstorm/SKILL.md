...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `the available user-input mechanism`** at key decision points throughout brainstorming:
   ...
   `the available user-input mechanism` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an the available user-input mechanism with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `the available user-input mechanism` for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `the available user-input mechanism` with these exact tab labels:
...
Present all three. Then use `the available user-input mechanism` to capture the selection.

**CRITICAL**: This MUST be a plain list call — no tabs, no form fields. Use exactly this structure:

```
the available user-input mechanism(
...
Ask these as `the available user-input mechanism` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `the available user-input mechanism`:
...
If the user selects B, C, or D, make the revision, then use `the available user-input mechanism` again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` with a Codex subagent in parallel before moving to Phase 5. Issue both subagent spawns simultaneously — do not wait for one before starting the other.**

- **`creative-director`** — gate **CD-PILLARS** (the installed Codex role-gate protocol)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (the installed Codex role-gate protocol)
...
Collect both verdicts, then present them together using a two-tab `the available user-input mechanism`:
...
- **Target platform**: Use `the available user-input mechanism` — "What platforms are you targeting for this game?"
  Options: `PC (Steam / Epic)` / `Mobile (iOS / Android)` / `Console` / `Web / Browser` / `Multiple platforms`
  Record the answer — it directly shapes the engine recommendation and will be passed to `$ai-game-studio:setup-engine`.
...
- **Engine experience**: Use `the available user-input mechanism` — "Do you already have an engine you work in?"
  ...
  - If "No preference" → tell them: "Run `$ai-game-studio:setup-engine` after this session — it will walk you through the full decision based on your concept and platform target." Do not make a recommendation here.
...
**After identifying biggest technical risks, spawn `technical-director` with a Codex subagent using gate TD-FEASIBILITY (the installed Codex role-gate protocol) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` with a Codex subagent using gate PR-SCOPE (the installed Codex role-gate protocol).**
...
4. **Generate the game concept document** using the template at
   `.ai-game-studio/templates/game-concept.md`. Fill in ALL sections from the
...
5. Use `the available user-input mechanism` for write approval:
...
If [B]: ask which section to revise using `the available user-input mechanism` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `the available user-input mechanism` — "Ready to write the updated concept document?"
Options: `[A] Yes — write it` / `[B] Revise another section`
Repeat until the user selects [A].

If yes, generate the document using the template at `.ai-game-studio/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
...
**Path A — Design-First** (recommended if the concept is well-defined):
   1. "Run `$ai-game-studio:setup-engine` to configure the engine and populate version-aware reference docs"
   2. "Run `$ai-game-studio:art-bible` to create the visual identity specification — do this BEFORE writing GDDs. **The art bible is required before the Technical Setup gate.** It gates asset production and shapes technical architecture decisions (rendering, VFX, UI systems)."
   3. "Use `$ai-game-studio:design-review design/gdd/game-concept.md` to validate concept completeness before going downstream"
   4. "Discuss vision with the `creative-director` agent for pillar refinement"
   5. "Decompose the concept into individual systems with `$ai-game-studio:map-systems` — maps dependencies, assigns priorities, and creates the systems index"
   6. "Author per-system GDDs with `$ai-game-studio:design-system` — guided, section-by-section GDD writing for each system identified in step 5"
   7. "Plan the technical architecture with `$ai-game-studio:create-architecture` — produces the master architecture blueprint and Required ADR list"
   8. "Record key architectural decisions with `$ai-game-studio:architecture-decision (×N)` — write one ADR per decision in the Required ADR list from `$ai-game-studio:create-architecture`"
   9. "Run `$ai-game-studio:architecture-review` — bootstraps the TR registry and Requirements Traceability Matrix from your GDDs and ADRs (required before the Pre-Production gate)"
   10. "Validate readiness to advance with `$ai-game-studio:gate-check` — phase gate before committing to production"

**Path B — Prototype-First** (use if the core mechanic is unproven or the concept needs validation):
   1. "Run `$ai-game-studio:setup-engine` to configure the engine"
   2. "Run `$ai-game-studio:prototype [core-mechanic]` — validate the core idea is fun before writing any GDDs (1–3 days throwaway code)"
   3. "If prototype PROCEEDS: run `$ai-game-studio:art-bible`, then continue with Path A steps 5–10 above, using prototype learnings to inform your GDDs"
   4. "If prototype PIVOTS: return to `$ai-game-studio:brainstorm` with the learnings and reshape the concept"
   5. "After full design and architecture, build the `$ai-game-studio:vertical-slice` to validate production readiness before committing to sprints"
...
1. `$ai-game-studio:setup-engine` — configure the engine and populate version-aware reference docs
2. `$ai-game-studio:art-bible` — establish visual identity before writing any GDDs
3. `$ai-game-studio:map-systems` — decompose the concept into individual systems with dependencies
4. `$ai-game-studio:design-system [first-system]` — author per-system GDDs in dependency order
5. `$ai-game-studio:create-architecture` — produce the master architecture blueprint
6. `$ai-game-studio:architecture-review` — bootstrap TR registry and Requirements Traceability Matrix
7. `$ai-game-studio:gate-check pre-production` — validate readiness before committing to production
