...
Before reading any full document, use text search to extract `## Summary` sections
from all GDDs and ADRs:

```
text search pattern="## Summary" glob="design/gdd/*.md" output_mode="content" -A 4
text search pattern="## Summary" glob="docs/architecture/adr-*.md" output_mode="content" -A 3
```

For `single-gdd [path]` mode: use the target GDD's summary to identify which
ADRs reference the same system (text search ADRs for the system name), then full-read
...
file discovery `production/epics/**/*.md` (excluding EPIC.md index files). For each
story file:
...
file discovery `tests/unit/**/*_test.*` and `tests/integration/**/*_test.*`.
Build an index: system → [test file paths].

For each test file path from Step 3b-1, confirm via file discovery whether the file
...
- text search all ADRs for API names listed in `deprecated-apis.md`
...
After completing the engine audit above, spawn the **primary engine specialist** with a Codex subagent for a domain-expert second opinion:
- Read `.ai-game-studio/project.json` `Engine Specialists` section to get the primary specialist
...
If [B]: display the complete proposed systems-index section, then re-ask with `the available user-input mechanism`.
...
For each gap:
  ❌ TR-[id]: [GDD] → [system] → [requirement]
     Suggested ADR: "$ai-game-studio:architecture-decision [suggested title]"
...
Use `the available user-input mechanism` for the write approval:
...
For `rtm` mode, use `the available user-input mechanism`:
...
> Last Updated: [date]
> Mode: $ai-game-studio:architecture-review rtm
...
```markdown
### [YYYY-MM-DD] — $ai-game-studio:architecture-review — 🔴 CONFLICT
...
`production/session-state/active.md`:
    ...
    ## Session Extract — $ai-game-studio:architecture-review [date]
    - Verdict: [PASS / CONCERNS / FAIL]
...
2. **Pre-gate checklist**: Check whether these exist via file discovery and mark each ✅ or ❌:
   - `tests/unit/` and `tests/integration/` directories — if ❌: run `$ai-game-studio:test-setup`
   - `.github/workflows/tests.yml` — if ❌: run `$ai-game-studio:test-setup`
   - `design/accessibility-requirements.md` — if ❌: run `$ai-game-studio:ux-design`
   - `design/ux/interaction-patterns.md` — if ❌: run `$ai-game-studio:ux-design`
   Present ❌ items as required steps before gate-check. Do not offer `$ai-game-studio:gate-check`
   as an option if any item is ❌ — offer the missing skill to run instead.
3. **Rerun trigger**: "Re-run `$ai-game-studio:architecture-review` after each new ADR is written
   to verify coverage improves"

Then close with `the available user-input mechanism` tailored to the pre-gate checklist state:
- If ADR gaps remain or any pre-gate item is ❌:
  - "Architecture review complete. What would you like to do next?"
    - [A] Write a missing ADR — open a fresh session and run `$ai-game-studio:architecture-decision [system]`
    - [B] Run `$ai-game-studio:test-setup` — required before gate-check (only show if test infrastructure is ❌)
    - [C] Run `$ai-game-studio:ux-design` — required before gate-check (only show if UX/accessibility files are ❌)
...
- If all pre-gate checklist items are ✅ and no blocking ADR gaps remain:
  - "Architecture review complete. All pre-gate items confirmed. What would you like to do next?"
    - [A] Run `$ai-game-studio:gate-check pre-production`
    - [B] Write a missing ADR — open a fresh session and run `$ai-game-studio:architecture-decision [system]`
...
3. **Offer options** via the available user-input mechanism with three choices:
...
5. **Use `the available user-input mechanism` for write approvals** — plain text "May I?" is not
