...
### Issues
- [N] hardcoded strings found in source code (run $ai-game-studio:localize scan)
...
Spawn `localization-lead` with a Codex subagent. Ask them to audit the following for cultural sensitivity across the target locales (read from `assets/data/strings/` and `assets/`):
...
file discovery `assets/audio/vo/[locale]/` for all `.wav`/`.ogg` files. Cross-reference against the VO manifest. Report:
...
text search `src/` for VO audio references. Verify each referenced path exists in `assets/audio/vo/[locale]/`. Report broken references.
...
Read `.ai-game-studio/project.json` to determine the engine. Then check:
...
[ ] $ai-game-studio:localize scan shows zero hardcoded strings
[ ] $ai-game-studio:localize validate shows no placeholder mismatches in source (en)
...
## Post-Freeze Changes
[Any strings added or modified after freeze are listed here automatically by $ai-game-studio:localize extract]
```
...
Spawn `localization-lead` with a Codex subagent with:
- The target locale(s) to QA
- The list of all screens/flows in the game (from `design/gdd/` or `$ai-game-studio:content-audit` output)
- The current `$ai-game-studio:localize validate` report
...
```
$ai-game-studio:localize scan            → find hardcoded strings
$ai-game-studio:localize extract         → build string table
$ai-game-studio:localize freeze          → lock source before sending to translators
$ai-game-studio:localize brief           → generate translator briefing document
[Send to translators]
$ai-game-studio:localize validate        → check returned translations
$ai-game-studio:localize cultural-review → flag culturally sensitive content
$ai-game-studio:localize rtl-check       → if shipping Arabic / Hebrew / Persian
$ai-game-studio:localize vo-pipeline     → if shipping dubbed VO
$ai-game-studio:localize qa              → full localization QA pass
```

After `qa` returns PASS for all shipping locales, include the QA report path when running `$ai-game-studio:gate-check release`.
