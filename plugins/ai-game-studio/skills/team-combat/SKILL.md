...
> "Usage: `$ai-game-studio:team-combat [combat feature description]` — Provide a description of the combat feature to design and implement (e.g., `melee parry system`, `ranged weapon spread`)."
...
- **engine specialist** (primary) — Validate architecture and implementation patterns are idiomatic for the engine (read from `.ai-game-studio/project.json` Engine Specialists section)
...
delegated to sub-agents spawned with a Codex subagent. Each sub-agent enforces the
...
- Run `$ai-game-studio:code-review` on the implemented combat code before closing stories.
- Run `$ai-game-studio:balance-check` to validate combat formulas and tuning values.
- Run `$ai-game-studio:team-polish` if VFX, audio, or performance polish is needed.
