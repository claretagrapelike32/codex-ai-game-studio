...
If a matching file is found, use `the available user-input mechanism`:
...
> "No sprint data found for [sprint/milestone]. Run `$ai-game-studio:sprint-status` to generate
> sprint data first, or provide the sprint details manually."

Then use `the available user-input mechanism` to present two options:
...
Run git log for the sprint period to understand what was actually committed and when. Use the available shell (which uses a platform-native shell; use PowerShell syntax on Windows and POSIX syntax on macOS/Linux):

```
Shell example (adapt syntax to the current platform): git log --oneline --since="4 weeks ago" 2>/dev/null || git log --oneline -20
```
...
If the user selects [A]: Proceed to invoke `$ai-game-studio:sprint-plan new`, passing the retrospective file path and a summary of the action items and velocity change so the sprint planner can reference them.

- If this was a milestone retrospective, run `$ai-game-studio:gate-check` to formally assess readiness for the next phase.
