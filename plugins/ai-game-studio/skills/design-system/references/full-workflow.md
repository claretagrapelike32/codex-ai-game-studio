...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `the available user-input mechanism`:
...
3. If no systems index exists, fail with:
   > "Usage: `$ai-game-studio:design-system <system-name>` — e.g., `$ai-game-studio:design-system movement`
   > Or to fill gaps in an existing GDD: `$ai-game-studio:design-system retrofit design/gdd/[system-name].md`
   > No systems index found. Run `$ai-game-studio:map-systems` first to map your systems and get the design order."
...
- **Game concept**: Read `design/gdd/game-concept.md` — fail if missing:
  > "No game concept found. Run `$ai-game-studio:brainstorm` first."
- **Systems index**: Read `design/gdd/systems-index.md` — fail if missing:
  > "No systems index found. Run `$ai-game-studio:map-systems` first to map your systems."
...
- **Related GDDs**: file discovery `design/gdd/*.md` and read any that are thematically related
...
- Read `.ai-game-studio/project.json` to identify the engine and version
...
- file discovery `docs/architecture/adr-*.md` and read any ADRs whose domain matches
...
> `$ai-game-studio:setup-engine` before moving to architecture if you haven't already."
...
Use the template structure from `.ai-game-studio/templates/game-design-document.md`:
...
> "Verdict: **BLOCKED** — skeleton creation declined. The design session cannot proceed without the skeleton file, as all subsequent phases use it as the base. Re-run `$ai-game-studio:design-system [system]` when ready to create the file."
Do not proceed to Section A.

After writing, update `production/session-state/active.md`:
- Use file discovery to check if the file exists.
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `the available user-input mechanism` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `the available user-input mechanism` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `the available user-input mechanism`. **NEVER use plain text. NEVER skip this step.**
...
7. **Write**: Use the available file-editing mechanism to replace the placeholder with the approved content.
   **CRITICAL**: Always include the section heading in the `old_string` to ensure
   ...
   same placeholder and the available file-editing mechanism requires a unique match. Use this pattern:
   ```
...
completed section name. Use file discovery to check if the file exists — use Write to create
...
- **ADR ref tab**: file discovery `docs/architecture/adr-*.md` and grep for the system name in the GDD Requirements section of any ADR. If a matching ADR is found → `[A] Yes — cite the ADR` recommended. If none found → `[B] No` recommended.
...
**Framing questions (ask BEFORE drafting)**: Use `the available user-input mechanism` with a multi-tab widget:
...
in `$ai-game-studio:architecture-decision`, not the GDD. The GDD describes behavior; the ADR
...
**Framing question (ask BEFORE drafting)**: Use `the available user-input mechanism`:
...
spawn `creative-director` with a Codex subagent:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents with a Codex subagent in parallel:
...
- Surface any disagreements between agents to the user via `the available user-input mechanism`
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents with a Codex subagent in parallel:
...
- Present the specialists' proposals to the user for review via `the available user-input mechanism`
...
For required systems: **spawn `art-director` with a Codex subagent** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
> "📌 **Asset Spec** — Visual/Audio requirements are defined. After the art bible is approved, run `$ai-game-studio:asset-spec system:[system-name]` to produce per-asset visual descriptions, dimensions, and generation prompts from this section."
...
> (Pre-Production), run `$ai-game-studio:ux-design` to create a UX spec for each screen or
...
Before finalizing the GDD, spawn `creative-director` with a Codex subagent using gate **CD-GDD-ALIGN** (the installed Codex role-gate protocol).
...
```
text search pattern="  - name: [candidate_name]" path="design/registry/entities.yaml"
```
...
> `$ai-game-studio:design-review design/gdd/[system-name].md`
>
> **Never run `$ai-game-studio:design-review` in the same session as `$ai-game-studio:design-system`.** The reviewing
...
**NEVER offer to run `$ai-game-studio:design-review` inline.** Always direct the user to a fresh window.
...
- "What's next?"
  - Options:
    - "Run `$ai-game-studio:consistency-check` — verify this GDD's values don't conflict with existing GDDs (recommended before designing the next system)"
    - "Design next system ([next-in-order])" — if undesigned systems remain
    - "Fix review findings" — if design-review flagged issues
    - "Stop here for this session"
    - "Run `$ai-game-studio:gate-check`" — if enough MVP systems are designed
...
**When delegating via Codex subagent mechanism**:
...
- The main session presents the agent's output to the user via `the available user-input mechanism`
- The user decides; the main session writes to file
...
2. **the available user-input mechanism** at every decision point (Explain -> Capture pattern):
...
> open a fresh Codex session and run `$ai-game-studio:design-system [system-name]` — it will
...
- Run `$ai-game-studio:design-review design/gdd/[system-name].md` in a **fresh session** to validate the completed GDD independently
- Run `$ai-game-studio:consistency-check` to verify this GDD's values don't conflict with other GDDs
- Run `$ai-game-studio:map-systems next` to move to the next highest-priority undesigned system
- Run `$ai-game-studio:gate-check pre-production` when all MVP GDDs are authored and reviewed
