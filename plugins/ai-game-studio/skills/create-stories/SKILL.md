---
name: create-stories
description: "Break a single epic into implementable story files. Reads the epic, its GDD, governing ADRs, and control manifest. Each story embeds its GDD requirement TR-ID, ADR guidance, acceptance criteria, story type, and test evidence path. Run after $ai-game-studio:create-epics for each epic."
---
...
**Previous step:** `$ai-game-studio:create-epics [system]`
**Next step after stories exist:** `$ai-game-studio:story-readiness [story-path]` then `$ai-game-studio:dev-story [story-path]`
...
in this skill — apply the check pattern from the installed Codex role-gate protocol
before every gate invocation.

- `$ai-game-studio:create-stories [epic-slug]` — e.g. `$ai-game-studio:create-stories combat`
- `$ai-game-studio:create-stories production/epics/combat/EPIC.md` — full path also accepted
- No argument — ask: "Which epic would you like to break into stories?"
  file discovery `production/epics/*/EPIC.md` and list available epics with their status.
...
> Check the filename in the epic's Governing ADRs list, or run `$ai-game-studio:architecture-decision`
...
The type determines what test evidence is required before `$ai-game-studio:story-done` can close the story.
...
- **Governing ADR**: which ADR governs how to implement this?
  ...
  - `Status: Proposed` → set story `Status: Blocked` with note: "BLOCKED: ADR-NNNN is Proposed — run `$ai-game-studio:architecture-decision` to advance it"
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` with a Codex subagent using gate **QL-STORY-READY** (the installed Codex role-gate protocol).
...
**Before generating test specs**: file discovery `production/qa/qa-plan-*.md` for the most recently modified file. If found, read it and check whether it contains test case specifications for the stories in this epic (look for story titles or slugs in the plan's Automated Tests Required section). If matching specs exist:
...
- If "Skip": leave `## QA Test Cases` with a placeholder: `*Test cases not yet defined — run $ai-game-studio:qa-plan to generate them.*`
...
> **Last Updated**: [set by $ai-game-studio:dev-story when implementation begins]
...
Use `the available user-input mechanism` to close with context-aware next steps:

Check:
- Are there other epics in `production/epics/` without stories yet? List them.
- Is this the last epic? If so, include `$ai-game-studio:sprint-plan` as an option.

Widget:
- Prompt: "[N] stories written to `production/epics/[epic-slug]/`. What next?"
- Options (include all that apply):
  - `[A] Start implementing — run $ai-game-studio:story-readiness [first-story-path]` (Recommended)
  - `[B] Create stories for [next-epic-slug] — run $ai-game-studio:create-stories [slug]` (only if other epics have no stories yet)
  - `[C] Plan the sprint — run $ai-game-studio:sprint-plan new` (only if all epics have stories)
...
- **Verdict: COMPLETE** — [N] stories written to `production/epics/[epic-slug]/`. Run `$ai-game-studio:story-readiness` → `$ai-game-studio:dev-story` to begin implementation.
