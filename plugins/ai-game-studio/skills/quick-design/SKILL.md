...
Full GDD authoring via `$ai-game-studio:design-system` is the heavyweight path. Use this skill
...
**When to run:** Anytime a change is too small for `$ai-game-studio:design-system` but too
...
and redirect to `$ai-game-studio:design-system` instead.

If there is no argument, ask the user to describe the change (plain text prompt), then classify it using the criteria above.

Present the inferred classification using `the available user-input mechanism`:
...
- Options:
  ...
  - `[F] This is too large — redirect me to $ai-game-studio:design-system`

If [F]: stop. Verdict: **REDIRECTED** — use `$ai-game-studio:design-system` for this change.
...
Present the draft to the user in full. Then use `the available user-input mechanism`:
...
- Options:
  ...
  - `[C] This grew too large — redirect to $ai-game-studio:design-system instead`

If [B]: collect the requested changes, revise the draft, and re-present this widget.
If [C]: stop. Verdict: **REDIRECTED** — use `$ai-game-studio:design-system` for this change.
...
Next step: This spec is ready for `$ai-game-studio:story-readiness` validation before
...
Quick Design Specs **bypass** `$ai-game-studio:design-review` and `$ai-game-studio:review-all-gdds` by
...
using `$ai-game-studio:design-system` to author a full GDD for this."

---

## Recommended Next Steps

- Run `$ai-game-studio:story-readiness [story-path]` to validate the story before implementation begins — reference this spec in the story's GDD Reference field
- Run `$ai-game-studio:dev-story [story-path]` to implement once the story passes readiness checks
- If the change is larger than expected, run `$ai-game-studio:design-system [system-name]` to author a full GDD instead
