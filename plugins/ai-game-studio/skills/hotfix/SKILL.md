...
If the check passes, use `the available user-input mechanism` before creating the branch:
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `qa-lead` with a Codex subagent with:
- The hotfix description and affected system
- The regression test results from Phase 5
- A list of all systems that touch the changed files (use text search to find callers)

Ask qa-lead: **Is a full smoke check sufficient, or does this fix require a targeted team-qa pass?**

Apply the verdict:
- **Smoke check sufficient** — run `$ai-game-studio:smoke-check` against the hotfix build. If PASS, proceed to Phase 6.
- **Targeted QA pass required** — run `$ai-game-studio:team-qa [affected-system]` scoped to the changed system only. If QA returns APPROVED or APPROVED WITH CONDITIONS, proceed to Phase 6.
- **Full QA required** — S1 fixes that touch core systems may require a full `$ai-game-studio:team-qa sprint`. This delays deployment but prevents a bad patch.
...
Next: $ai-game-studio:bug-report verify [BUG-ID] after deploy to confirm resolution
```
...
After deploying, run `$ai-game-studio:bug-report verify [BUG-ID]` to confirm the fix resolved the issue in the deployed build.

If VERIFIED FIXED: run `$ai-game-studio:bug-report close [BUG-ID]` to formally close it.
If STILL PRESENT: the hotfix failed — immediately re-open, assess rollback, and escalate.

Schedule a post-incident review within 48 hours using `$ai-game-studio:retrospective hotfix`.

Use `the available user-input mechanism`:
- Prompt: "Hotfix complete. What's the next step?"
- Options:
  - `[A] Run $ai-game-studio:smoke-check to verify the fix`
  - `[B] Run $ai-game-studio:patch-notes to document this hotfix`
  - `[C] Stop here`
