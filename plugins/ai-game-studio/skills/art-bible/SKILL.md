---
name: art-bible
description: "Guided, section-by-section Art Bible authoring. Creates the visual identity specification that gates all asset production. Run after $ai-game-studio:brainstorm is approved and before $ai-game-studio:map-systems or any GDD authoring begins."
---
...
> "No game concept found. Run `$ai-game-studio:brainstorm` first — the art bible is authored after the game concept is approved."
...
**Retrofit mode detection**: file discovery `design/art/art-bible.md`. If the file exists:
- Read it in full
...
Read `.ai-game-studio/project.json` if it exists — extract performance budgets and engine for asset standard constraints.
...
Use `the available user-input mechanism` with two tabs:
...
**Agent delegation (MANDATORY)**: Spawn `art-director` with a Codex subagent:
...
Present the art-director's draft to the user. Use `the available user-input mechanism`:
...
**Agent delegation**: Spawn `art-director` with a Codex subagent with Visual Identity Statement and mood targets. Ask: "Define the shape language for this game. Connect each shape principle back to the visual identity statement and a specific game pillar. Explain what these shape choices communicate to the player emotionally."
...
**Agent delegation**: Spawn `art-director` with a Codex subagent with Visual Identity Statement and mood targets. Ask: "Design the color system for this game. Every semantic color assignment must be explained — why does this color mean danger/safety/reward in this world? Identify which color pairs might fail colorblind players and specify what backup cues are needed."
...
**Agent delegation**: Spawn `art-director` with a Codex subagent with sections 1–4. Ask: "Define character design direction for this game. Cover: visual archetype for the player character (if any), distinguishing feature rules per character type (how do players tell enemies/NPCs/allies apart at a glance?), expression/pose style targets (stiff/expressive/realistic/exaggerated), and LOD philosophy (how much detail is preserved at game camera distance?)."

Write the approved section to file.

### Section 6: Environment Design Language

**Agent delegation**: Spawn `art-director` with a Codex subagent with sections 1–4. Ask: "Define the environment design language for this game. Cover: architectural style and its relationship to the world's culture/history, texture philosophy (painted vs. PBR vs. stylized — why this choice for this game?), prop density rules (sparse/dense — what drives the choice per area type?), and environmental storytelling guidelines (what visual details should tell the story without text?)."
...
Collect both. If they conflict (e.g., art-director wants elaborate diegetic UI but ux-designer flags it would reduce combat readability), surface the conflict explicitly with both positions. Do NOT silently resolve — use `the available user-input mechanism` to let the user decide.
...
- **`technical-artist`**: Engine-specific hard constraints — poly count budgets per asset category, texture memory limits, material slot counts, importer constraints, anything from the performance budgets in `.ai-game-studio/project.json`
...
After all sections are complete (or the scoped set from Phase 1 is complete), spawn `creative-director` with a Codex subagent using gate **AD-ART-BIBLE** (the installed Codex role-gate protocol).
...
- Does `.ai-game-studio/project.json` contain a configured engine (not `[TO BE CONFIGURED]`)? → setup-engine is done, skip that option
...
- Do GDDs exist (check above)? → include $ai-game-studio:consistency-check option

Use `the available user-input mechanism` for next steps. Only include options that are genuinely next based on the state check above:

**Option pool — include only if not already done:**
- `[_] Run $ai-game-studio:map-systems — decompose the concept into systems before writing GDDs` (skip if systems-index.md exists)
- `[_] Run $ai-game-studio:setup-engine — configure the engine (asset standards may need revisiting after engine is set)` (skip if engine configured)
- `[_] Run $ai-game-studio:design-system — start the first GDD` (skip if any GDDs exist)
- `[_] Run $ai-game-studio:review-all-gdds — cross-GDD consistency check (required before Technical Setup gate)` (skip if gdd-cross-review-*.md exists)
- `[_] Run $ai-game-studio:asset-spec — generate per-asset visual specs and AI generation prompts from approved GDDs` (include if GDDs exist)
- `[_] Run $ai-game-studio:consistency-check — scan existing GDDs against the art bible for visual direction conflicts` (include if GDDs exist)
- `[_] Run $ai-game-studio:create-architecture — author the master architecture document (next Technical Setup step)`
- `[_] Stop here`
...
> **Always include** `$ai-game-studio:create-architecture` and Stop here as options — these are always valid next steps once the art bible is complete.
...
- Run `$ai-game-studio:map-systems` to decompose the concept into game systems before authoring GDDs
- Run `$ai-game-studio:setup-engine` if the engine is not yet configured (asset standards may need revisiting after engine selection)
- Run `$ai-game-studio:design-system [first-system]` to start authoring per-system GDDs
- Run `$ai-game-studio:consistency-check` once GDDs exist to validate them against the art bible's visual rules
- Run `$ai-game-studio:create-architecture` to produce the master architecture document
