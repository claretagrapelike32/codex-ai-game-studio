...
**Argument:** `the invocation arguments` (blank = ask user via the available user-input mechanism)
...
- **No argument** — use `the available user-input mechanism`:
...
**Date**: [date]
**Generated by**: $ai-game-studio:qa-plan
**Scope**: [N stories across [N systems]]
**Engine**: [engine name from .ai-game-studio/project.json, or "Not configured"]
...
*Smoke tests are verified by the developer via `$ai-game-studio:smoke-check`. Reference this
...
- [ ] Smoke check passes (run `$ai-game-studio:smoke-check sprint` before QA hand-off)
- [ ] No regressions introduced
- [ ] Code reviewed (via `$ai-game-studio:code-review` or documented peer review)
- [ ] Story file updated to `Status: Complete` (via `$ai-game-studio:story-done`)
````
...
then ask two questions together using `the available user-input mechanism`:
...
options:
  ...
  - "Also back-fill test case specs into each story file's ## QA Test Cases section (Recommended — enables $ai-game-studio:dev-story and $ai-game-studio:code-review traceability)"
```
...
- Once all sprint stories are implemented, run `$ai-game-studio:smoke-check sprint` to gate QA hand-off — not yet, only after implementation is complete
- For Logic/Integration stories, create the test files at the listed paths
  before marking stories done — `$ai-game-studio:story-done` checks for them"
...
- Use `the available user-input mechanism` for scope selection when no argument is provided.
