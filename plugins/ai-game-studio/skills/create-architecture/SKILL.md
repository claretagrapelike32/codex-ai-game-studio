...
**Distinct from `$ai-game-studio:architecture-decision`**: ADRs record individual point decisions.
...
> "No engine is configured. Run `$ai-game-studio:setup-engine` first. Architecture cannot be
...
3. `.ai-game-studio/project.json` — naming conventions, performance budgets,
...
- `$ai-game-studio:architecture-decision [title]` → covers: TR-[id], TR-[id]

**Core Layer:**
- `$ai-game-studio:architecture-decision [title]` → covers: TR-[id]
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `the available user-input mechanism`:
...
Apply gate **TD-ARCHITECTURE** (the installed Codex role-gate protocol) as a self-review. Check all four criteria from that gate definition against the completed document.
...
**Step 2 — Spawn `lead-programmer` with a Codex subagent using gate LP-FEASIBILITY (the installed Codex role-gate protocol):**
...
Use `the available user-input mechanism` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `the available user-input mechanism`:
...
**1. `$ai-game-studio:architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence: what it defines and what it unblocks.]

**2. `$ai-game-studio:architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence.]

**3. `$ai-game-studio:architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence.]
...
> **Required before `$ai-game-studio:gate-check [stage]`:**
> - [ ] Accept ADRs: [list Proposed ADR IDs that must be Accepted]
> - [ ] Write ADRs: [list ADR IDs that must still be written]
> - [ ] Run `$ai-game-studio:test-setup` — scaffolds `tests/unit/`, `tests/integration/`, CI workflow, and an example test file
> - [ ] Run `$ai-game-studio:ux-design` — creates `design/ux/interaction-patterns.md` and `design/accessibility-requirements.md`
>
> Run `$ai-game-studio:gate-check [stage]` when all boxes are checked.

If nothing is blocking, write instead:
> No blockers — run `$ai-game-studio:gate-check [stage]` now.
...
- Run `$ai-game-studio:architecture-decision [title]` for each required ADR listed in Phase 6 — Foundation layer ADRs first
- Run `$ai-game-studio:architecture-review` — bootstraps the Requirements Traceability Matrix and TR registry from the ADRs just written. Required before the Pre-Production gate.
- Run `$ai-game-studio:test-setup` to scaffold `tests/unit/`, `tests/integration/`, CI workflow, and an example test (required for gate-check)
- Run `$ai-game-studio:ux-design` to initialize `design/ux/interaction-patterns.md` and `design/accessibility-requirements.md` (required for gate-check)
- Run `$ai-game-studio:create-control-manifest` once the required ADRs are written to produce the layer rules manifest
- Run `$ai-game-studio:gate-check pre-production` when all required ADRs, `$ai-game-studio:test-setup`, and `$ai-game-studio:ux-design` are complete
