...
whether the core idea is worth designing, run `$ai-game-studio:prototype` (concept prototype) instead.
...
- Naming conventions from `.ai-game-studio/project.json`
...
Read `.ai-game-studio/templates/vertical-slice-report.md` to get the report structure.
...
- `full` → spawn `creative-director` with a Codex subagent using gate **CD-PLAYTEST**
...
Recommended next steps:
- `$ai-game-studio:create-epics layer:foundation` — plan Foundation layer epics
- `$ai-game-studio:create-epics layer:core` — plan Core layer epics
- `$ai-game-studio:create-stories [epic-slug]` — break each epic into implementable stories
- `$ai-game-studio:sprint-plan` — plan the first sprint using velocity data from the slice
- `$ai-game-studio:gate-check pre-production` — formally advance the stage to Production

**Playtest note:** `$ai-game-studio:gate-check` will look for documented playtest evidence.
...
differently. When `$ai-game-studio:vertical-slice` is next run after a PIVOT, check the
`prototypes/` directory for a `PIVOT-NOTE.md` — use it to frame the new validation
question and inform scope decisions.

- Revise affected GDDs with `$ai-game-studio:design-system [mechanic]`
- Address architecture issues via `$ai-game-studio:architecture-decision`
- Then re-run `$ai-game-studio:vertical-slice` to validate the revised direction
...
- Return to `$ai-game-studio:brainstorm` with what you learned
- Or run `$ai-game-studio:prototype [new-concept]` to test a new direction cheaply first
