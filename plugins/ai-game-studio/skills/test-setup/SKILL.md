...
1. **Read engine config**:
   ...
   - If engine is not configured (`[TO BE CONFIGURED]`), stop:
     "Engine not configured. Run `$ai-game-studio:setup-engine` first, then re-run `$ai-game-studio:test-setup`."
...
2. **Check for existing test infrastructure**:
   ...
   - file discovery `tests/gdunit4_runner.gd` (Godot) or `tests/EditMode/` (Unity) or
...
3. **Report findings**:
   ...
   - If everything already exists AND `force` argument was not passed:
     "Test infrastructure appears to be in place. Re-run with `$ai-game-studio:test-setup force`
...
tests/
  ...
  smoke/          # Critical path test list for $ai-game-studio:smoke-check gate
...
```gdscript
# GdUnit4 test runner — invoked by CI and $ai-game-studio:smoke-check
...
**Run via**: `$ai-game-studio:smoke-check` (which reads this file)
...
3. Run `$ai-game-studio:qa-plan sprint` before your first sprint to classify stories and set
   test evidence requirements
4. `$ai-game-studio:smoke-check` before every QA hand-off

Gate note: $ai-game-studio:gate-check Technical Setup → Pre-Production now requires:
- tests/ directory with unit/ and integration/ subdirectories
- .github/workflows/tests.yml
- At least one example test file
Run $ai-game-studio:test-setup and write one example test before advancing.
...
- **Engine detection is non-negotiable** — if the engine is not configured,
  stop and redirect to `$ai-game-studio:setup-engine`. Do not guess.
