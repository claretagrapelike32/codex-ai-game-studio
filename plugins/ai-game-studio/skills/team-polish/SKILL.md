...
> Usage: `$ai-game-studio:team-polish [feature or area]` — specify the feature or area to polish (e.g., `combat`, `main menu`, `inventory system`, `level-1`). Do not use `the available user-input mechanism` here; output the guidance directly.
...
- Profile the target feature/area using `$ai-game-studio:perf-profile`
...
- If READY FOR RELEASE: run `$ai-game-studio:release-checklist` for the final pre-release validation.
- If NEEDS MORE WORK: schedule remaining issues in `$ai-game-studio:sprint-plan update` and re-run `$ai-game-studio:team-polish` after fixes.
- Run `$ai-game-studio:gate-check` for a formal phase gate verdict before handing off to release.
