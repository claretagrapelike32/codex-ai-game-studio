...
Read `.ai-game-studio/project.json` to determine:
...
Spawn `security-engineer` with a Codex subagent. Pass:
...
text search patterns: `File.open`, `load`, `deserialize`, `JSON.parse`, `from_json`, `read_file` — check each for validation.
...
text search for: `recv`, `receive`, `PacketPeer`, `socket`, `NetworkedMultiplayerPeer`, `rpc`, `rpc_id` — check each call site for validation.
...
text search for: `get_input`, `Input.get_`, `input_map`, user-facing text fields — check validation.
...
text search for: `api_key`, `secret`, `password`, `token`, `private_key`, `DEBUG`, `print(` in release-facing code.
...
file discovery for: `addons/`, `plugins/`, `third_party/`, `vendor/` — list all external dependencies.
...
**Audited by**: security-engineer via $ai-game-studio:security-audit
...
Run `$ai-game-studio:security-audit` again after remediating any CRITICAL or HIGH findings.
...
After remediating findings, re-run: `$ai-game-studio:security-audit quick` to confirm CRITICAL/HIGH items are resolved before running `$ai-game-studio:gate-check release`.

If CRITICAL findings exist:
> "⛔ CRITICAL security findings must be resolved before any public release. Do not proceed to `$ai-game-studio:launch-checklist` until these are addressed."

If no CRITICAL/HIGH findings:
> "✅ No blocking security findings. Report written to `production/security/`. Include this path when running `$ai-game-studio:gate-check release`."
