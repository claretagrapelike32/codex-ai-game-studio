...
- `$ai-game-studio:test-flakiness scan` — scan all available CI logs in `.github/` or
  standard log output directories
- `$ai-game-studio:test-flakiness registry` — read existing regression-suite.md quarantine
...
> 3. Run `$ai-game-studio:test-flakiness registry` to review tests already flagged as flaky
...
- text search for `<testcase name=` to get test names
- text search for `<failure` or `<error` to identify failures
- Parse `classname` and `name` attributes for full test identifiers

**Plain text logs**:
- text search for pass/fail patterns:
...
Use text search to check the test file for timing calls, randf, global state access,
