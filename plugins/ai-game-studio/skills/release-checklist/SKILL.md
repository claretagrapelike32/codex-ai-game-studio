...
> **Explicit invocation only**: This skill should only run when the user explicitly requests it with `$ai-game-studio:release-checklist`. Do not auto-invoke based on context matching.
...
- Run `$ai-game-studio:gate-check` for a formal phase gate verdict before proceeding to release.
- Coordinate final sign-offs via `$ai-game-studio:team-release`.
