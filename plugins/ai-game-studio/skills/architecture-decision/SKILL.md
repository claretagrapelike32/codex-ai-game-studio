...
(e.g., `$ai-game-studio:architecture-decision retrofit docs/architecture/adr-0001-event-system.md`):

Enter **retrofit mode**:

1. Read the existing ADR file completely.
2. Identify which template sections are present by scanning headings:
   - `## Status` — **BLOCKING if missing**: `$ai-game-studio:story-readiness` cannot check ADR acceptance
...
5. If yes:
   ...
   - Append each missing section to the ADR file using the available file-editing mechanism.
...
7. Suggest: "Run `$ai-game-studio:architecture-review` to re-validate coverage now that this ADR
...
6. **Display a knowledge gap warning** before proceeding if the domain carries
   ...
   Run `$ai-game-studio:setup-engine` first, or tell me which engine you are using."
...
a **confirm/adjust** prompt using `the available user-input mechanism` — not open-ended questions.
...
decisions remain, present each one as a separate `the available user-input mechanism` with the proposed
...
5.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** with a Codex subagent to validate the drafted ADR:
...
5.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` with a Codex subagent using gate **TD-ADR** (the installed Codex role-gate protocol):
...
5. **Write approval** — Use `the available user-input mechanism`:
...
Ask using `the available user-input mechanism`:
...
After the ADR is written (and registry optionally updated), close with `the available user-input mechanism`.
...
[N] Start writing GDDs — run `$ai-game-studio:design-system [first-undesigned-system]` (only show if all prerequisite ADRs are written)
[N+1] Stop here for this session
```

If there are no remaining priority ADRs and no undesigned GDD systems, offer only "Stop here" and suggest running `$ai-game-studio:architecture-review` in a fresh session.
...
> and run `$ai-game-studio:architecture-review`.
>
> **Never run `$ai-game-studio:architecture-review` in the same session as `$ai-game-studio:architecture-decision`.**
