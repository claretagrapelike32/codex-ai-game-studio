...
**When to run:**
- After `$ai-game-studio:test-setup` scaffolds the framework (first time)
...
**Modes:**
- `$ai-game-studio:test-helpers [system-name]` — generate helpers for a specific system
  (e.g., `$ai-game-studio:test-helpers combat`)
- `$ai-game-studio:test-helpers all` — generate helpers for all systems with test files
- `$ai-game-studio:test-helpers scaffold` — generate only the base helper library (no
...
Read `.ai-game-studio/project.json` and extract:
- `Engine:` value
- `Language:` value
- `Framework:` from the Testing section

If engine is not configured: "Engine not configured. Run `$ai-game-studio:setup-engine` first."
...
```
file discovery pattern="tests/**/*_test.*" (all test files)
```
...
## Generated by $ai-game-studio:test-helpers combat on [date].
...
- Run `$ai-game-studio:test-setup` if the test framework has not been scaffolded yet.
- Use `$ai-game-studio:dev-story` to implement stories — helpers reduce boilerplate in new test files.
- Run `$ai-game-studio:skill-test` to validate other skills that may need helper coverage.
