...
Then use `the available user-input mechanism` to collect the **estimated fix effort**:
...
- Run `$ai-game-studio:sprint-plan` to schedule high-priority debt items into the next sprint.
- Run `$ai-game-studio:tech-debt report` at the start of each sprint to track debt trends over time.
