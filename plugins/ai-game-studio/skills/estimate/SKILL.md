...
- If confidence is Low: recommend a time-boxed spike (`$ai-game-studio:prototype`) before committing.
- If the task is > 10 days: recommend breaking it into smaller stories via `$ai-game-studio:create-stories`.
- To schedule the task: run `$ai-game-studio:sprint-plan update` to add it to the next sprint.
