...
`$ai-game-studio:smoke-check` verifies that test files **exist** and **pass**. This skill
...
**Logic stories**: file discovery `tests/unit/[system]/[story-slug]_test.*`
  - If not found, also try: text search in `tests/unit/[system]/` for files
    containing the story slug

**Integration stories**: file discovery `tests/integration/[system]/[story-slug]_test.*`
  - Also check `production/session-logs/` for playtest records mentioning the story

**Visual/Feel and UI stories**: file discovery `production/qa/evidence/[story-slug]-evidence.*`

**Config/Data stories**: file discovery `production/qa/smoke-*.md` (any smoke check report)
...
Heuristics:
- text search test file for "zero", "max", "null", "empty", "min", "invalid",
...
in the evidence doc. If referenced, file discovery for them to confirm they exist.
...
- For BLOCKING items: "These must be resolved before `$ai-game-studio:story-done` can mark the
  story Complete. Would you like to address any of them now?"
- For thin assertions: "Consider running `$ai-game-studio:test-helpers [system]` to see
