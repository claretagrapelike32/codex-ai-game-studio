...
> Usage: `$ai-game-studio:team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`). Do not use `the available user-input mechanism` here; output the guidance directly.
...
Spawn the **primary engine specialist** in parallel (from `.ai-game-studio/project.json` Engine Specialists) to validate the integration approach:
...
- Use `$ai-game-studio:dev-story` to implement the audio manager and event system once the design is approved.
- Run `$ai-game-studio:asset-audit` after audio assets are created to verify naming and format compliance.
