...
> **Explicit invocation only**: This skill should only run when the user explicitly requests it with `$ai-game-studio:launch-checklist`. Do not auto-invoke based on context matching.
...
- Run `$ai-game-studio:gate-check` to get a formal PASS/CONCERNS/FAIL verdict before launch.
- Coordinate sign-offs via `$ai-game-studio:team-release`.
