...
**Default use** — run right after `$ai-game-studio:brainstorm` and `$ai-game-studio:setup-engine`, before writing
...
before committing to Production, run `$ai-game-studio:vertical-slice` instead.
...
Otherwise, use `the available user-input mechanism` to confirm intent before proceeding:
...
[answer]." Suggest next step: `$ai-game-studio:map-systems` or `$ai-game-studio:design-system [mechanic]`.
...
consider timing it to a game jam (Ludum Dare, GMTK Game Jam, file discoveryal Game Jam). Jams
...
Read `AGENTS.md` and `.ai-game-studio/project.json` for the engine and
language in use.
...
Assess which path best fits the hypothesis, then use `the available user-input mechanism` with your
...
Read `.ai-game-studio/templates/prototype-report.md` to get the report structure.
...
- `full` → spawn `creative-director` with a Codex subagent using gate **CD-PLAYTEST** if
...
1. `$ai-game-studio:design-review design/gdd/game-concept.md` — validate the concept doc against what the prototype revealed
2. `$ai-game-studio:gate-check` — confirm readiness to advance to Systems Design
3. `$ai-game-studio:art-bible` — define visual identity (optional but worth doing before GDDs)
4. `$ai-game-studio:map-systems` — decompose the concept into all game systems
5. `$ai-game-studio:design-system [mechanic]` — GDD for each MVP system; use prototype learnings
   in the Tuning Knobs and Formulas sections
6. `$ai-game-studio:review-all-gdds` — cross-system consistency check
...
the revised hypothesis for the next prototype. When `$ai-game-studio:prototype` is next run, check
...
- Run `$ai-game-studio:prototype [revised-concept]` to test the adjusted direction
- Or `$ai-game-studio:brainstorm [hint]` if the concept needs more fundamental rethinking
...
- Run `$ai-game-studio:brainstorm open` or `$ai-game-studio:brainstorm [new-hint]` to explore a different concept
...
2. **Choose path** — same the available user-input mechanism widget as Phase 3 (HTML / Engine / Paper).
