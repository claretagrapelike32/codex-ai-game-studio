...
**If no argument is provided**, do not fail — ask instead. Use `the available user-input mechanism`:
...
- **Game concept**: Read `design/gdd/game-concept.md` — if missing, warn:
  > "No game concept found. Run `$ai-game-studio:brainstorm` first to establish the game's
...
> "Player journey map not yet created. Template available at `.ai-game-studio/templates/player-journey.md`. Run `$ai-game-studio:ux-design` Phase 2b or create it manually to establish player context for this screen."

### 2c: GDD UI Requirements

file discovery `design/gdd/*.md` and grep for `UI Requirements` sections. Read any GDD whose
...
file discovery `design/ux/*.md` and note which screens already have specs. For screens that
...
Read `.ai-game-studio/project.json` and extract the `## Input & Platform`
...
>
> (Run `$ai-game-studio:setup-engine` to save this permanently so you won't be asked again.)
...
file discovery `design/ux/[filename].md` (where `[filename]` is the resolved output path from Phase 1).
...
2. **Questions**: Ask what is needed to draft this section. Use `the available user-input mechanism`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `the available user-input mechanism` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `the available user-input mechanism`:
...
7. **Write**: Use `the available user-input mechanism`: "May I write the [section name] section to `[filepath]`?"
...
- Use `the available user-input mechanism` to capture the choice:
...
- Use `the available user-input mechanism`: "Want an ASCII wireframe as part of this spec?"
...
> "Accessibility tier not yet defined — consider WCAG-AA as a baseline. Run `$ai-game-studio:gate-check` to see whether this blocks any phase gates."
...
Write at least 5 specific, testable criteria that a QA tester can verify without reading any other design document. These become the pass/fail conditions for `$ai-game-studio:story-done`.
...
Use `the available user-input mechanism` to confirm:
...
Use `the available user-input mechanism` to step through items in groups of 3-4, not all at once.
...
file discovery `design/ux/*.md` (excluding `interaction-patterns.md`) and read the Component
...
Work through patterns in groups. Use `the available user-input mechanism`:
...
> "This spec should be validated with `$ai-game-studio:ux-review` before it enters the
> implementation pipeline. The Pre-Production gate requires all key screen specs
> to have a review verdict."

Then use `the available user-input mechanism`:
- "Run `$ai-game-studio:ux-review [filename]` now, or do something else first?"
  - Options:
    - "Run `$ai-game-studio:ux-review` now — validate this spec"
...
`$ai-game-studio:ux-review` on all completed specs before running `$ai-game-studio:gate-check pre-production`."
...
When delegating to another agent via the Codex subagent mechanism:
...
- Run `$ai-game-studio:ux-review [filename]` to validate this spec before it enters the implementation pipeline
- Run `$ai-game-studio:ux-design [next-screen]` to continue designing remaining screens or flows
- Run `$ai-game-studio:gate-check pre-production` once all key screens have approved UX specs
