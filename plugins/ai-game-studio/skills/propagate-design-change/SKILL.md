...
**Usage:** `$ai-game-studio:propagate-design-change design/gdd/combat-system.md`
...
> "Usage: `$ai-game-studio:propagate-design-change design/gdd/[system].md`
...
Spawn `technical-director` with a Codex subagent using gate **TD-CHANGE-IMPACT** (the installed Codex role-gate protocol).
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; use `the available user-input mechanism` with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
...
- **ADRs marked Superseded**: "Run `$ai-game-studio:architecture-decision [title]` to write the
  replacement ADR. Then re-run `$ai-game-studio:propagate-design-change` to verify coverage."
- **ADRs to update in place**: List the specific fields to update in each ADR
- **If many ADRs affected**: "Run `$ai-game-studio:architecture-review` after all ADRs are updated
