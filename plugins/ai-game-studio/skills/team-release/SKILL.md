...
2. If a version is found: report "No version argument provided — inferred [version] from milestone data. Proceeding." Then confirm with `the available user-input mechanism`: "Releasing [version]. Is this correct?"
3. If no version is discoverable: use `the available user-input mechanism` to ask "What version number should be released? (e.g., v1.0.0)" and wait for user input before proceeding. Do NOT default to a hardcoded version string.
...
- Generate the release checklist using `$ai-game-studio:release-checklist`
...
- Generate changelog using `$ai-game-studio:changelog`
- Deploy to staging for final smoke test
- Deploy to production
- Human team action: Monitor dashboards and error rates for 48 hours post-release. Schedule a follow-up retrospective using `$ai-game-studio:retrospective` at the 48-hour mark.

Delegate to **community-manager** (in parallel with deployment):
- Finalize patch notes using `$ai-game-studio:patch-notes [version]`
...
- Run `$ai-game-studio:retrospective` if significant issues occurred during the release.
