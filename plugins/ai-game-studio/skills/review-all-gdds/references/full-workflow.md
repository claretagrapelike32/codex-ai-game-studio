...
**This is distinct from `$ai-game-studio:design-review`**, which reviews one GDD for internal
...
- Before `$ai-game-studio:create-architecture` begins (architecture built on inconsistent GDDs
...
Run `$ai-game-studio:consistency-check` after this review to populate the registry."
...
> then re-run `$ai-game-studio:review-all-gdds`."
...
- The engine name and version from `.ai-game-studio/project.json` and `docs/engine-reference/[engine]/VERSION.md`
...
Use `the available user-input mechanism` for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second `the available user-input mechanism`:
...
append to `production/session-state/active.md`:
    ...
    ## Session Extract — $ai-game-studio:review-all-gdds [date]
    - Verdict: [PASS / CONCERNS / FAIL]
    - GDDs reviewed: [N]
...
After all file writes are complete, use `the available user-input mechanism` for a closing widget.
...
- Are any GDDs in the "Flagged for Revision" table? → offer $ai-game-studio:design-review option for each
- Read systems-index.md for the next system with Status: Not Started → offer $ai-game-studio:design-system option
- Is the verdict PASS or CONCERNS? → offer $ai-game-studio:gate-check or $ai-game-studio:create-architecture
...
- `[_] Run $ai-game-studio:design-review [flagged-gdd-path] — address flagged warnings` (one per flagged GDD, if any)
- `[_] Run $ai-game-studio:design-system [next-system] — next in design order` (always include, name the actual system)
- `[_] Run $ai-game-studio:create-architecture — begin architecture (verdict is PASS/CONCERNS)` (include if verdict is not FAIL)
- `[_] Run $ai-game-studio:gate-check — validate Systems Design phase gate` (include if verdict is PASS)
- `[_] Stop here`
