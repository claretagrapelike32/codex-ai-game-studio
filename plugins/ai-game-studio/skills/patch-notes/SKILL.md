...
- Read the internal changelog at `production/releases/[version]$ai-game-studio:changelog.md` if it exists
...
> "No changelog data found for [version]. Run `$ai-game-studio:changelog [version]` first to generate the
> internal changelog, then re-run `$ai-game-studio:patch-notes [version]`."
...
1. Check `.ai-game-studio/project.json` for any "tone", "voice", or "style"
   fields or sections.
...
1. file discovery for `docs/patch-notes-template.md` and `.ai-game-studio/templates/release-notes.md`.
...
if needed. Also write to `production/releases/[version]$ai-game-studio:patch-notes.md` as the
...
- Run `$ai-game-studio:release-checklist` to verify all other release gates are met before publishing.
