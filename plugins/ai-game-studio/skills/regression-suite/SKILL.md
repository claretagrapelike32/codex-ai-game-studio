...
- Before a release gate (`$ai-game-studio:gate-check polish` requires regression suite exists)
...
**Modes:**
- `$ai-game-studio:regression-suite update` — scan new bug fixes this sprint and check
  for regression test presence; add new tests to the suite manifest
- `$ai-game-studio:regression-suite audit` — full audit of all GDD critical paths vs.
  existing test coverage; flag paths with no regression test
- `$ai-game-studio:regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
- No argument — if a sprint is clearly active (sprint plan exists with in-progress stories), run `update`. If ambiguous or no active sprint is detected, use `the available user-input mechanism`:
...
file discovery `production/qa/bugs/*.md` and filter for bugs with a `Status: Closed`
...
1. text search `tests/unit/[system]/` and `tests/integration/[system]/` for file names
...
2. text search `tests/unit/[system]/` and `tests/integration/[system]/` for a test
...
- GDD sections added or revised since the regression suite was last updated
  (use text search on GDD file modification hints if available, or ask the user)
...
- For each HIGH priority gap: "Consider creating the missing regression test
  before the next sprint. Run `$ai-game-studio:test-helpers` to scaffold the test file."
...
- If coverage drift detected: "Regression suite may be drifting. Consider
  running `$ai-game-studio:regression-suite audit` at the next sprint boundary."
