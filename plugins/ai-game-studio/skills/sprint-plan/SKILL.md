...
- If the file doesn't exist and this is a `new` sprint: use `the available user-input mechanism`:
...
- [ ] Smoke check passed (`$ai-game-studio:smoke-check sprint`)
- [ ] QA sign-off report: APPROVED or APPROVED WITH CONDITIONS (`$ai-game-studio:team-qa sprint`)
...
3. Ask the user what to change: stories to add, remove, reprioritize, or re-estimate. Use `the available user-input mechanism` to gather changes.
...
`$ai-game-studio:sprint-status`, `$ai-game-studio:story-done`, and `$ai-game-studio:help` without markdown parsing.
...
```yaml
# Auto-generated by $ai-game-studio:sprint-plan. Updated by $ai-game-studio:story-done and $ai-game-studio:dev-story.
# DO NOT edit manually — use $ai-game-studio:story-done to update story status.
#
...
Before finalising the sprint plan, spawn `producer` with a Codex subagent using gate **PR-SPRINT** (the installed Codex role-gate protocol).
...
> **Scope check:** If this sprint includes stories added beyond the original epic scope, run `$ai-game-studio:scope-check [epic]` to detect scope creep before implementation begins.
...
Use `file discovery` to look for `production/qa/qa-plan-sprint-[N].md` or any file in `production/qa/` referencing this sprint number.
...
>
> Run `$ai-game-studio:qa-plan sprint` now, before starting any implementation. It takes one session and produces the test case requirements each story needs."

Use `the available user-input mechanism`:
- Prompt: "No QA plan found for this sprint. How do you want to proceed?"
- Options:
  - `[A] Run $ai-game-studio:qa-plan sprint now — I'll do that before starting implementation (Recommended)`
  - `[B] Skip for now — I understand QA sign-off will be blocked at the Production → Polish gate`

If [A]: close with "Sprint plan written. Run `$ai-game-studio:qa-plan sprint` next — then begin implementation."
If [B]: add a warning block to the sprint plan document:

```markdown
> ⚠️ **No QA Plan**: This sprint was started without a QA plan. Run `$ai-game-studio:qa-plan sprint`
...
- `$ai-game-studio:qa-plan sprint` — **required before implementation begins** — defines test cases per story so developers implement against QA specs, not a blank slate
- `$ai-game-studio:story-readiness [story-file]` — validate a story is ready before starting it
- `$ai-game-studio:dev-story [story-file]` — begin implementing the first story
- `$ai-game-studio:sprint-status` — check progress mid-sprint
- `$ai-game-studio:scope-check [epic]` — verify no scope creep before implementation begins
...
This file is read by `$ai-game-studio:sprint-plan`, `$ai-game-studio:story-readiness`, `$ai-game-studio:story-done`, and other skills at startup.
