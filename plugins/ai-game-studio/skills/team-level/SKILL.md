...
**Decision Points:** At each step transition, use `the available user-input mechanism` to present
...
Spawn all three agents simultaneously — issue all three subagent spawns before waiting for any result.
...
**Gate**: Use `the available user-input mechanism` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
Use `the available user-input mechanism` with options:
...
- (b) Pause and run `$ai-game-studio:team-level [area-name]` first to establish that area

Do NOT invent content for the missing adjacent area.

**Gate**: Use `the available user-input mechanism` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `the available user-input mechanism` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `the available user-input mechanism` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
After all subagent outputs are collected, spawn `level-designer` with a Codex subagent to compile and write the final document:
...
to sub-agents spawned with a Codex subagent. Each sub-agent enforces the "May I write to [path]?"
...
- Run `$ai-game-studio:design-review design/levels/[level-name].md` to validate the completed level design doc.
- Run `$ai-game-studio:dev-story` to implement level content once the design is approved.
- Run `$ai-game-studio:qa-plan` to generate a QA test plan for this level.
