...
the `$ai-game-studio:team-ui` pipeline.
...
- **Specific file path** (e.g., `$ai-game-studio:ux-review design/ux/inventory.md`): validate
  that one document
...
1. **Input & Platform config**: Read `.ai-game-studio/project.json` and
...
[For APPROVED]: This spec is ready for handoff to `$ai-game-studio:team-ui` Phase 2
(Visual Design).

[For NEEDS REVISION]: Address the [N] blocking issues above, then re-run
`$ai-game-studio:ux-review`.

[For MAJOR REVISION NEEDED]: The spec has fundamental gaps in [areas].
Recommend returning to `$ai-game-studio:ux-design` to rework [sections].
```
...
- For **APPROVED**: suggest running `$ai-game-studio:team-ui` to begin implementation coordination
- For **NEEDS REVISION**: offer to help fix specific gaps ("Would you like me to
  ...
  instruction
- For **MAJOR REVISION NEEDED**: suggest returning to `$ai-game-studio:ux-design` with the
