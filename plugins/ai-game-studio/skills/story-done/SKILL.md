...
3. If multiple in-progress stories are found, use `the available user-input mechanism`:
...
- **File existence check**: `file discovery` for files the story said would be created.
- **Test pass check**: if a test file path is mentioned, run it with the available shell.
- **No hardcoded values check**: `text search` for numeric literals in gameplay code
  paths that should be in config files.
- **No hardcoded strings check**: `text search` for player-facing strings in `src/`
  that should be in localization files.
- **Dependency check**: if a criterion says "depends on X", check that X exists.

### Manual verification with confirmation (use `the available user-input mechanism`)
...
Batch up to 4 manual verification questions into a single `the available user-input mechanism` call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Unit test**: check `tests/unit/` for a test file or function name that
     matches the criterion's subject (use `file discovery` and `text search`)
   - **Integration test**: check `tests/integration/` similarly
   - **Manual confirmation**: if the criterion was verified via `the available user-input mechanism`
...
exact required file path. Use `file discovery` to check that exact path. If the exact path is not
...
required path. Use `file discovery` to check that exact path first, then search
...
- If found: read the file and check the sign-off table for unchecked boxes. text search for lines matching `| .* | .* | .* | \[ \] Approved` (a sign-off row with an unchecked checkbox). If any unchecked sign-off rows are found: flag as **ADVISORY** — "Evidence file found at `[path]` but [N] sign-off(s) are still pending (shown as `[ ] Approved` in the sign-off table). Obtain required sign-offs before final closure. Note: for solo developers, all roles may be signed off by the same person."
...
If none: flag as **ADVISORY** — "No smoke check report found. Run `$ai-game-studio:smoke-check`."
...
1. **GDD rules check**: Using the current requirement text from `tr-registry.yaml`
   ...
   `text search` the implemented files for key function names, data structures, or class
...
2. **Manifest version staleness check**: Compare the `Manifest Version:` date
   ...
   - If the story's version is older → flag as ADVISORY:
     ...
     is v[current-date]. New rules may apply. Run $ai-game-studio:story-readiness to check.`
...
3. **ADR constraints check**: Read the referenced ADR's Decision section. Check
   ...
   exists). `text search` for patterns explicitly forbidden in the ADR.

4. **Hardcoded values check**: `text search` the implemented files for numeric literals
...
After completing the deviation checks in Phase 4, spawn `qa-lead` with a Codex subagent using gate **QL-TEST-COVERAGE** (the installed Codex role-gate protocol).
...
- `lean` → use `the available user-input mechanism` before proceeding:
  - Prompt: "Code review is skipped in lean mode. Did you run `$ai-game-studio:code-review` on the implemented files?"
  - Options:
    - `Yes — $ai-game-studio:code-review passed or was approved with suggestions`
    - `No — skipping code review for this story`
    - `No — I'll run $ai-game-studio:code-review before the sprint close-out`
  - Record the answer in the completion notes (Phase 7). All three options proceed to Phase 6.
- `full` → spawn as normal.

Spawn `lead-programmer` with a Codex subagent using gate **LP-CODE-REVIEW** (the installed Codex role-gate protocol).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them via `the available user-input mechanism`:
...
Use `the available user-input mechanism` before writing anything:
...
`production/session-state/active.md`:
    ...
    ## Session Extract — $ai-game-studio:story-done [date]
    - Verdict: [COMPLETE / COMPLETE WITH NOTES / BLOCKED]
...
Run `$ai-game-studio:story-readiness [path]` to confirm a story is implementation-ready
before starting.
```
...
1. `$ai-game-studio:smoke-check sprint` — verify the critical path still works end-to-end
2. `$ai-game-studio:team-qa sprint` — full QA cycle: test case execution, bug triage, sign-off report
3. `$ai-game-studio:retrospective` — capture what went well, what didn't, and action items for the next sprint
4. `$ai-game-studio:gate-check` — advance to the next phase once QA approves (only if advancing a phase)
5. `$ai-game-studio:sprint-plan new` — plan the next sprint, incorporating velocity data and retrospective action items

Do not run `$ai-game-studio:gate-check` until `$ai-game-studio:team-qa` returns APPROVED or APPROVED WITH CONDITIONS.
```
...
- Use `the available user-input mechanism` for the code review prompt and for batching manual
...
- Run `$ai-game-studio:story-readiness [next-story-path]` to validate the next story before starting implementation
- If all Must Have stories are complete: run `$ai-game-studio:smoke-check sprint` → `$ai-game-studio:team-qa sprint` → `$ai-game-studio:gate-check`
- If tech debt was logged: track it via `$ai-game-studio:tech-debt` to keep the register current
