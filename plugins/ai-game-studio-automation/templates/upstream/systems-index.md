...
- [ ] Design MVP-tier systems first (use `$ai-game-studio:design-system [system-name]`)
- [ ] Run `$ai-game-studio:design-review` on each completed GDD
- [ ] Run `$ai-game-studio:gate-check pre-production` when MVP systems are designed
- [ ] Validate the highest-risk systems with `$ai-game-studio:vertical-slice` before committing to Production
