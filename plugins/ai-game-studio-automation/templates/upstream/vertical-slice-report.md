...
**Playtest note:** Run `$ai-game-studio:playtest-report` to structure additional session data
before running `$ai-game-studio:gate-check pre-production`.

**Next steps:**
1. `$ai-game-studio:gate-check pre-production` — formally advance to Production
2. `$ai-game-studio:create-epics layer:foundation` — plan Foundation layer epics
3. `$ai-game-studio:create-epics layer:core` — plan Core layer epics
4. `$ai-game-studio:sprint-plan` — use velocity data from this report in the estimate
...
**Architecture decisions to revisit:** [List — use `$ai-game-studio:architecture-decision` to update]
**Core loop change needed:** [What specifically to change]

**Next steps:**
1. `$ai-game-studio:design-system [mechanic]` — revise affected GDDs
2. `$ai-game-studio:architecture-decision [decision]` — address architecture issues
3. `$ai-game-studio:vertical-slice` — re-validate after revisions
...
**Next step:** `$ai-game-studio:brainstorm` to explore a new direction, or `$ai-game-studio:prototype [new-concept]`
