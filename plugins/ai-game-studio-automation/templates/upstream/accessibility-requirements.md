...
>
> **When to update**: After each `$ai-game-studio:gate-check` pass, after any accessibility
...
| Brightness/gamma controls | Basic | file discoveryal | Not Started | Exposed in graphics settings. Include a reference calibration image (a gradient or symbol barely visible at correct calibration). Range: -50% to +50% from default. |
...
| Mono audio option | Comprehensive | file discoveryal audio output | Not Started | Folds stereo/spatial audio to mono. Preserves volume balance between channels rather than summing to full volume on both sides. Essential for players with single-sided deafness. |
