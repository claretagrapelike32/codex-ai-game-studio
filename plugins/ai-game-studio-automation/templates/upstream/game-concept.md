...
- [ ] Fill in AGENTS.md technology stack based on engine choice (`$ai-game-studio:setup-engine`)
- [ ] Create game pillars document (`$ai-game-studio:design-review` to validate)
- [ ] **Prototype core idea** (`$ai-game-studio:prototype [core-mechanic]`) — before writing GDDs, validate the concept is worth designing
- [ ] If prototype PROCEEDS: Decompose concept into systems (`$ai-game-studio:map-systems`)
- [ ] Design each system (`$ai-game-studio:design-system [system-name]`) — use prototype learnings in Tuning Knobs and Formulas sections
- [ ] Build vertical slice in Pre-Production (`$ai-game-studio:vertical-slice`) — validate full game loop before committing to Production
- [ ] Validate core loop with playtest (`$ai-game-studio:playtest-report`)
- [ ] Plan first milestone (`$ai-game-studio:sprint-plan new`)
