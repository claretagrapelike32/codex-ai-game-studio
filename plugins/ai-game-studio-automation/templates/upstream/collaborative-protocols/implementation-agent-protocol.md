<!-- Adapted from the pinned upstream source at 984023ddac0d5e27624f2baacde6105e45de375f under MIT; see parity/ledger.json. -->
...
2. **Ask architecture questions:**
   ...
   - *Use `the available user-input mechanism` to batch constrained architecture questions*
...
5. **Get approval before writing files:**
   ...
   - Wait for "yes" before using the available file-editing mechanism

6. **Complete the story with `$ai-game-studio:story-done`:**
   - When implementation (and tests, if written) is complete, invoke `$ai-game-studio:story-done [story-file-path]`
   - This verifies acceptance criteria, checks for GDD/ADR deviations, prompts code review, and updates the story status
   - Do NOT mark a story complete manually — always go through `$ai-game-studio:story-done`
   - If no story file exists for this work (ad-hoc task), offer `$ai-game-studio:code-review` directly instead
...
File is ready. Would you like me to:
 ...
 C) Run $ai-game-studio:code-review for architecture validation"
...
You: [creates tests/combat/test_damage_calculator.gd]
     "Created test file with 6 test cases covering the acceptance criteria from the design doc.
      ...
      Running $ai-game-studio:story-done to verify acceptance criteria and close out the story."

[$ai-game-studio:story-done runs — verifies criteria, checks deviations, prompts code review, updates story status]
```
...
- Story completion is explicit — use `$ai-game-studio:story-done` to close every story, never assume done because code is written

#### Structured Decision UI

Use the `the available user-input mechanism` tool for architecture decisions and next-step choices.
Follow the **Explain → Capture** pattern:

1. **Explain first** — Describe the architectural options and trade-offs in
   conversation text.
2. **Capture the decision** — Call `the available user-input mechanism` with concise option labels.
...
**Example — architecture questions (batch):**
  ...
  the available user-input mechanism with questions:
