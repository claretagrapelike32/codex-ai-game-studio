...
1. **Ask clarifying questions:**
   ...
   - *Use `the available user-input mechanism` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `the available user-input mechanism` to capture the decision*
...
You: [uses Write tool]
     "Created design/gdd/crafting-system.md. Would you like me to run $ai-game-studio:design-review to validate it?"
```
...
Use the `the available user-input mechanism` tool to present decisions as a selectable UI instead of
...
- Batch up to 4 independent questions in a single `the available user-input mechanism` call
...
- When running as a Codex subagent (tool may not be available) — structure your
  text output so the orchestrator can present options via the available user-input mechanism
